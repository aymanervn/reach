...
extern "C"
{
    ...
    typedef struct reach_media_controls_port
    {
        ...
        /* Fast core read: no thumbnail I/O or image decoding. */
        reach_result (*get_state)(void *userdata, reach_media_controls_state *out_state);
        /* A current result owns one reference to a nonzero cover_icon_id. A
         * zero id is a valid missing/failed cover. current is zero when the
         * requested media generation became obsolete during the read. */
