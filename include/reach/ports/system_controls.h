...
typedef void (*reach_system_controls_change_callback)(
    void *user,
...
typedef struct reach_system_controls_port {
    ...
    reach_result (*get_network_state)(
        void *userdata,
        reach_network_state *out_state
    );

    reach_result (*get_bluetooth_state)(
        void *userdata,
        reach_bluetooth_state *out_state
    );

    reach_result (*set_bluetooth_enabled)(
        void *userdata,
        int32_t enabled
    );
    ...
    reach_result (*set_battery_saver_enabled)(
        void *userdata,
        int32_t enabled
    );

    reach_result (*get_brightness_state)(
        void *userdata,
        reach_brightness_state *out_state
    );

    reach_result (*set_brightness_level)(
        void *userdata,
        float level
    );

    reach_result (*open_project_menu)(
        void *userdata
    );

    reach_result (*start_watching)(
        void *userdata,
