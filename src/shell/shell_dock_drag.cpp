...
void reach_shell_begin_dock_drag(reach_shell *shell, size_t index, const reach_ui_event *event)
{
    ...
    shell->dock_drag.grab_offset_x = (float)event->x - (shell->layout.dock.bounds.x + box_x);
...
reach_result reach_shell_update_dock_drag(reach_shell *shell, const reach_ui_event *event)
{
    ...
    if (target != REACH_MAX_PINNED_APPS && target != shell->dock_drag.target_index)
    {
    ...
    current = reach_shell_find_dock_order_key(shell, shell->dock_drag.pinned,
                                              shell->dock_drag.pin_id, shell->dock_drag.window);

    if (current != REACH_MAX_PINNED_APPS && shell->feedback.dock_index != current)
    {
...
reach_result reach_shell_end_dock_drag(reach_shell *shell)
{
    ...
    size_t target_pinned_index =
        dragged_pinned ? reach_shell_pinned_order_index(shell, pin_id) : REACH_MAX_PINNED_APPS;
    ...
    if (moved && target_index < shell->layout.dock.app_slot_count)
    {
        ...
        reach_float_animation_start(&shell->dock_drag.snap_animation, shell->dock_drag.x, target_x,
                                    0.12);
    ...
    {
        return reach_shell_schedule_move_pin(shell, pin_id, target_pinned_index);
    }
