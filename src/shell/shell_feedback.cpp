...
static void reach_shell_start_dock_click_feedback(
    reach_shell *shell,
    size_t index,
    float target_opacity)
{
    ...
    reach_float_animation_start(
        ...
        shell->feedback.dock_opacity.value,
        target_opacity,
        0.055);
...
void reach_shell_release_dock_item(reach_shell *shell)
{
    ...
    if (shell->feedback.dock_index != REACH_SHELL_DOCK_FEEDBACK_NONE) {
        reach_shell_start_dock_click_feedback(
            shell,
            shell->feedback.dock_index,
            0.0f);
    }
...
void reach_shell_press_tray_button(reach_shell *shell)
{
    ...
    reach_shell_start_dock_click_feedback(
        shell,
        REACH_SHELL_DOCK_FEEDBACK_TRAY_BUTTON,
        0.50f);
}
...
void reach_shell_press_quick_settings_button(reach_shell *shell)
{
    ...
    reach_shell_start_dock_click_feedback(
        shell,
        REACH_SHELL_DOCK_FEEDBACK_QUICK_SETTINGS_BUTTON,
        0.50f);
}
...
void reach_shell_press_power_button(reach_shell *shell)
{
    ...
    reach_shell_start_dock_click_feedback(
        shell,
        REACH_SHELL_DOCK_FEEDBACK_POWER_BUTTON,
        0.50f);
}

static void reach_shell_start_tray_click_feedback(
    reach_shell *shell,
    size_t index,
    float target_opacity)
{
    ...
    reach_float_animation_start(
        ...
        shell->feedback.tray_opacity.value,
        target_opacity,
        0.055);
...
void reach_shell_release_tray_item(reach_shell *shell)
{
    ...
    if (shell->feedback.tray_index != REACH_MAX_TRAY_ITEMS) {
        reach_shell_start_tray_click_feedback(
            shell,
            shell->feedback.tray_index,
            0.0f);
    }
}
