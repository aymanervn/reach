...
reach_result reach_shell_render_quick_settings_surface(reach_shell *shell)
{
    ...
    return reach_shell_render_popup_surface(shell, &shell->quick_settings,
                                                 ...
                                                 const reach_launcher_layout *layout)
{
    ...
    if (shell == nullptr || layout == nullptr ||
        shell->launcher.renderer.ops.begin_frame == nullptr)
    {
        return REACH_OK;
    }
...
reach_result reach_shell_render_context_menu_surface(reach_shell *shell)
{
    ...
    if (shell->context_menu.renderer.ops.begin_frame(shell->context_menu.renderer.backend) !=
        REACH_OK)
    {
        return REACH_ERROR;
    }
    ...
    reach_result result =
        shell->context_menu.renderer.ops.execute(shell->context_menu.renderer.backend, &commands);
    if (result != REACH_OK)
    {
        return result;
    }
