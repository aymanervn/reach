                                                  ...
                                                  reach_shell **out_shell)
{
    ...
    shell->launcher_scrollbar_drag.grab_offset = 0.0f;
    ...
    if (result == REACH_OK && shell->config_store.ops.load != nullptr)
    {
        ...
        if (shell->config_store.ops.load(shell->config_store.store, &snapshot) == REACH_OK)
        {
            if (snapshot.dock_height > 0.0f)
                shell->ui.dock.height = snapshot.dock_height;
            if (snapshot.dock_width > 0.0f)
                shell->ui.dock.width = snapshot.dock_width;
            shell->ui.dock.icon_size =
                reach_theme_icon_box_size(shell->theme, shell->ui.dock.height);
            (void)reach_ui_state_set_pinned_apps(&shell->ui, snapshot.pinned_apps,
...
reach_result reach_shell_start(reach_shell *shell)
{
    ...
    if (shell->dock_reveal_edge.ops.set_callback != nullptr)
    {
        result = shell->dock_reveal_edge.ops.set_callback(shell->dock_reveal_edge.edge,
                                                          reach_shell_on_dock_reveal_edge, shell);
        if (result != REACH_OK)
        {
            return result;
        }
