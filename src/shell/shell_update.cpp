...
static int32_t
reach_shell_float_animation_active(const reach_float_animation *animation) {
  return animation != nullptr &&
         animation->elapsed_seconds < animation->duration_seconds;
}

static void
reach_shell_dispatch_surface_events(reach_surface_runtime *surface) {
...
static int32_t
reach_shell_surface_has_pending_events(const reach_surface_runtime *surface) {
...
static size_t reach_shell_min_size(size_t a, size_t b) { return a < b ? a : b; }

static float reach_shell_clamp_float(float value, float min_value,
                                     float max_value) {
  if (value < min_value) {
    return min_value;
  }
...
static int32_t reach_shell_dock_key_equal(int32_t a_pinned, uint32_t a_pin_id,
                                          uintptr_t a_window, int32_t b_pinned,
                                          uint32_t b_pin_id,
                                          uintptr_t b_window) {
...
uint32_t reach_shell_dock_item_pin_id(const reach_shell *shell, size_t index) {
  return shell != nullptr
             ? reach_dock_feature_model_item_pin_id(&shell->dock_model, index)
             : 0;
}
...
int32_t reach_shell_dock_item_matches_key(const reach_shell *shell,
  ...
  reach_dock_order_key key = {pinned, pin_id, window};
  return reach_dock_feature_model_item_matches_key(&shell->dock_model, index,
                                                   key);
}

int32_t reach_shell_dock_icon_size_px(const reach_shell *shell) {
  const reach_theme *theme = shell != nullptr && shell->theme != nullptr
                                 ? shell->theme
                                 : reach_theme_default();
...
static int32_t reach_shell_nonempty_text_equals(const uint16_t *a,
                                                const uint16_t *b) {
...
static int32_t
reach_shell_dock_window_matches_pinned(void *user,
  ...
  if (reach_shell_nonempty_text_equals(pinned_app->app_user_model_id,
...
reach_result reach_shell_refresh_open_windows(reach_shell *shell,
  ...
  size_t count =
      shell->window_manager.ops.window_count(shell->window_manager.manager);
  for (size_t index = 0;
       index < count && shell->open_window_count < REACH_MAX_PINNED_APPS;
       ++index) {
    reach_window_snapshot snapshot = {};
    if (shell->window_manager.ops.window_at(shell->window_manager.manager,
                                            index, &snapshot) != REACH_OK ||
        snapshot.path[0] == 0) {
      continue;
    }
  ...
  if (changed) {
    ...
    for (size_t index = 0; index < shell->open_window_count; ++index) {
      int32_t dock_item_changed =
          old_windows[index] != shell->open_windows[index].id ||
          !reach_shell_utf16_equal(old_paths[index],
      ...
      if (dock_item_changed ||
          ...
          old_bounds[index].bottom !=
              shell->open_windows[index].bounds.bottom ||
          !reach_shell_utf16_equal(old_titles[index],
                                   shell->open_windows[index].title)) {
        changed = 1;
      }
    }
  }
...
void reach_shell_build_dock_items(reach_shell *shell,
  ...
  reach_dock_feature_model_build_items(
      &shell->dock_model, shell->ui.pinned_apps, shell->ui.pinned_app_count,
      shell->open_windows, shell->open_window_count,
      reach_shell_dock_window_matches_pinned, shell);
  ...
  const reach_theme *theme =
      shell->theme != nullptr ? shell->theme : reach_theme_default();
  ...
  float dock_width = ceilf(icon_size * (float)(count + 3) + clock_width +
                           separator_width + gap * (float)(count + 5));

  if (count == 0) {
    dock_width =
        ceilf(icon_size * 3.0f + clock_width + separator_width + gap * 4.0f);
  }
  ...
  layout->quick_settings_button.x =
      layout->system_separator.x - gap - icon_size;
...
static void reach_shell_copy_ascii_to_utf16(uint16_t *dst, size_t dst_count,
                                            const char *src) {
...
static void reach_shell_update_clock_text(reach_shell *shell) {
  ...
  char time_text[32] = {};
  char date_text[64] = {};
  snprintf(time_text, sizeof(time_text), "%d:%02d %s", hour, local.tm_min,
           suffix);
  if (local.tm_mon < 0 || local.tm_mon > 11 || local.tm_wday < 0 ||
      local.tm_wday > 6) {
    return;
  }
  snprintf(date_text, sizeof(date_text), "%.3s %d, %.3s", months[local.tm_mon],
...
reach_result reach_shell_reload_pins(reach_shell *shell) {
  ...
  for (size_t order_index = 0; order_index < old_order_count; ++order_index) {
    ...
    old_order_pin_ids[order_index] =
        shell->dock_model.order[order_index].pin_id;
    old_order_windows[order_index] =
        shell->dock_model.order[order_index].window;
    if (old_order_pinned[order_index]) {
      for (size_t pin_index = 0; pin_index < shell->ui.pinned_app_count;
           ++pin_index) {
        if (shell->ui.pinned_apps[pin_index].id ==
            old_order_pin_ids[order_index]) {
  ...
  result = reach_ui_state_set_pinned_apps(&shell->ui, snapshot.pinned_apps,
  ...
  for (size_t order_index = 0; order_index < shell->dock_model.order_count;
       ++order_index) {
...
void reach_shell_seed_or_apply_wallpaper(reach_shell *shell,
  ...
  int32_t changed = reach_wallpaper_seed_or_apply(
      &shell->wallpaper_service, &shell->wallpaper_surface,
      snapshot->wallpaper_path, 260, snapshot->monitor_wallpaper_paths,
      REACH_MAX_WALLPAPER_MONITORS, shell->wallpaper_state.path, 260);
  if (changed && shell->config_store.ops.save != nullptr) {
    (void)shell->config_store.ops.save(shell->config_store.store, snapshot);
  }
}

static int32_t reach_shell_pinned_apps_equal(const reach_pinned_app_model *a,
                                             size_t a_count,
  ...
  for (size_t index = 0; index < a_count; ++index) {
    if (a[index].id != b[index].id ||
        ...
        !reach_shell_utf16_equal(a[index].app_user_model_id,
                                 b[index].app_user_model_id)) {
      return 0;
    }
  }
...
reach_result reach_shell_reload_config(reach_shell *shell) {
  ...
  if (!reach_shell_pinned_apps_equal(
          shell->ui.pinned_apps, shell->ui.pinned_app_count,
...
void reach_shell_reload_wallpaper(reach_shell *shell, int32_t force) {
  ...
  reach_config_snapshot snapshot = {};
  if (shell->config_store.ops.load(shell->config_store.store, &snapshot) !=
      REACH_OK) {
    return;
  }
  ...
  if (!force &&
      reach_shell_path_equals(shell->wallpaper_state.path, new_path)) {
    return;
  }

  reach_copy_utf16(shell->wallpaper_state.path, 260, new_path);
  if (new_path[0] != 0 &&
      shell->wallpaper_surface.ops.set_wallpaper != nullptr) {
    (void)shell->wallpaper_surface.ops.set_wallpaper(
        shell->wallpaper_surface.surface, new_path);
  } else if (new_path[0] == 0 &&
             shell->wallpaper_surface.ops.clear != nullptr) {
...
static int32_t
reach_shell_popup_blocks_dock_autohide(const reach_shell *shell) {
  return shell != nullptr &&
...
int32_t reach_shell_should_auto_hide_dock(const reach_shell *shell) {
  ...
  if (shell->window_manager.ops.dock_should_auto_hide != nullptr) {
    return shell->window_manager.ops.dock_should_auto_hide(
        shell->window_manager.manager);
  }
  if (shell->window_manager.ops.foreground_is_maximized != nullptr) {
    return shell->window_manager.ops.foreground_is_maximized(
        shell->window_manager.manager);
  }
  return 0;
}

static int32_t reach_shell_get_cursor_position(reach_shell *shell,
                                               reach_point_i32 *out_cursor) {
  if (shell == nullptr ||
      shell->input_source.ops.get_pointer_position == nullptr ||
      out_cursor == nullptr) {
    return 0;
  }
  return shell->input_source.ops.get_pointer_position(
             shell->input_source.source, out_cursor) == REACH_OK;
}

static int32_t reach_shell_point_in_rect(reach_point_i32 point,
                                         reach_rect_f32 rect) {
...
reach_shell_dock_reveal_bounds(reach_rect_f32 shown_bounds,
...
static int32_t
reach_shell_cursor_in_dock_reveal_bounds(reach_shell *shell,
  ...
  return reach_shell_point_in_rect(
      cursor, reach_shell_dock_reveal_bounds(shown_bounds, monitor_bounds));
}

static int32_t
reach_shell_popup_blocks_dock_reveal_edge(const reach_shell *shell) {
  if (shell == nullptr) {
    return 1;
  }

  return shell->ui.launcher.open || shell->switcher_state.open ||
         shell->tray_state.popup_open || shell->quick_settings_open ||
         shell->context_menu_state.open;
}

static int32_t reach_shell_reveal_edge_rect_equal(reach_rect_f32 a,
                                                  reach_rect_f32 b) {
...
void reach_shell_sync_dock_reveal_edge(reach_shell *shell,
  ...
  int32_t should_auto_hide =
      shell->ui.dock.auto_hide && reach_shell_should_auto_hide_dock(shell);
  ...
  if (should_show) {
    if (!shell->dock_reveal.edge_bounds_valid ||
        !reach_shell_reveal_edge_rect_equal(shell->dock_reveal.edge_bounds,
                                            edge_bounds)) {
      if (shell->dock_reveal_edge.ops.set_bounds != nullptr &&
          shell->dock_reveal_edge.ops.set_bounds(shell->dock_reveal_edge.edge,
                                                 edge_bounds) == REACH_OK) {
        shell->dock_reveal.edge_bounds = edge_bounds;
        shell->dock_reveal.edge_bounds_valid = 1;
      }
    }

    if (!shell->dock_reveal.edge_visible &&
        shell->dock_reveal_edge.ops.show != nullptr &&
        shell->dock_reveal_edge.ops.show(shell->dock_reveal_edge.edge) ==
            REACH_OK) {
...
reach_rect_f32 reach_shell_apply_dock_animation(reach_shell *shell,
  ...
  if (!should_auto_hide) {
    ...
    if (reach_shell_cursor_in_dock_reveal_bounds(shell, shown_bounds,
                                                 monitor_bounds)) {
      shell->dock_reveal.active = 1;
    }
  } else if (!popup_blocks_autohide && shell->dock_reveal.requested) {
    shell->dock_reveal.active = reach_shell_cursor_in_dock_reveal_bounds(
        shell, shown_bounds, monitor_bounds);
    shell->dock_reveal.requested = 0;
...
void reach_shell_apply_dock_width_animation(reach_shell *shell,
  ...
  const reach_theme *theme =
      shell->theme != nullptr ? shell->theme : reach_theme_default();
  float gap = shell->ui.dock.gap;
  layout->power_button.x = layout->bounds.x + layout->bounds.width -
                           layout->power_button.width - gap;
  layout->clock.x = layout->power_button.x - gap - theme->dock_clock_width;
  layout->system_separator.x =
      layout->clock.x - gap - theme->dock_system_separator_width;
  layout->quick_settings_button.x =
      layout->system_separator.x - gap - layout->quick_settings_button.width;
  layout->tray_button.x =
      layout->quick_settings_button.x - layout->tray_button.width;
}

static reach_result reach_shell_refresh_monitor_layout(reach_shell *shell) {
  if (shell == nullptr || !shell->dirty.monitors ||
  ...
  if (shell->monitors.list == nullptr ||
      shell->monitors.ops.refresh == nullptr ||
      shell->monitors.ops.count == nullptr ||
      shell->monitors.ops.get == nullptr) {
    return REACH_OK;
  }
  ...
  const reach_monitor_info *monitor =
      shell->monitors.ops.get(shell->monitors.list, 0);
  if (monitor == nullptr) {
    shell->dirty.monitors = 0;
    return REACH_OK;
  }
  ...
  if (!shell->wallpaper_state.bounds_valid ||
      !reach_shell_rect_equal(shell->wallpaper_state.bounds,
                              wallpaper_bounds)) {
...
float reach_shell_dock_drag_clamped_x(const reach_shell *shell,
  ...
  const reach_theme *theme = shell != nullptr && shell->theme != nullptr
                                 ? shell->theme
                                 : reach_theme_default();
  return reach_dock_drag_clamped_x(
      theme, layout, cursor_x,
...
size_t reach_shell_find_dock_item_key(const reach_shell *shell, int32_t pinned,
  ...
  for (size_t index = 0; index < shell->dock_model.item_count; ++index) {
    if (reach_shell_dock_item_matches_key(shell, index, pinned, pin_id,
                                          window)) {
      return index;
    }
  }
...
size_t reach_shell_dock_reorder_target(const reach_shell *shell,
  ...
  return shell != nullptr && shell->has_layout
             ? reach_dock_reorder_target(&shell->dock_model,
                                         &shell->layout.dock, current_index,
                                         dragged_box_x)
             : REACH_MAX_PINNED_APPS;
}
...
size_t reach_shell_pinned_order_index(const reach_shell *shell,
                                      uint32_t pin_id) {
  return shell != nullptr ? reach_dock_feature_model_pinned_order_index(
                                &shell->dock_model, pin_id)
                          : REACH_MAX_PINNED_APPS;
}
...
float reach_shell_dock_item_current_x(const reach_shell *shell,
                                      ...
                                      size_t index) {
  if (shell == nullptr || layout == nullptr ||
      index >= shell->dock_model.item_count ||
  ...
  if ((shell->dock_item_x_animating[index] ||
       shell->dock_item_x_valid[index]) &&
      reach_shell_dock_key_equal(shell->dock_item_x_pinned[index],
...
static void reach_shell_start_dock_item_x_animation(reach_shell *shell,
                                                    size_t index, float from,
                                                    float to) {
  ...
  reach_float_animation_start(&shell->dock_item_x_animations[index], from, to,
                              0.12);
  shell->dock_item_x_valid[index] = 1;
...
void reach_shell_rebuild_dock_items_with_animations(reach_shell *shell,
  ...
  const reach_dock_layout *old_layout =
      shell->has_layout ? &shell->layout.dock : layout;
  for (size_t index = 0; index < old_count; ++index) {
  ...
  for (size_t index = 0; index < shell->dock_model.item_count; ++index) {
    ...
    for (size_t old_index = 0; old_index < old_count; ++old_index) {
      if (reach_shell_dock_key_equal(
              old_pinned[old_index], old_pin_ids[old_index],
              old_windows[old_index], shell->dock_model.items[index].pinned,
              pin_id, shell->dock_model.items[index].window)) {
  ...
  for (size_t index = shell->dock_model.item_count;
       index < REACH_MAX_PINNED_APPS; ++index) {
...
static int32_t
reach_shell_can_move_dock_without_redraw(const reach_shell *shell) {
  if (shell == nullptr) {
    return 0;
  }
...
static reach_result
reach_shell_move_dock_animation_frame(reach_shell *shell,
                                      double delta_seconds) {
  ...
  reach_result result = reach_shell_apply_window_state(
      &shell->dock.window, dock_bounds, 1.0f, &shell->dock.last_bounds,
...
reach_result reach_shell_update(reach_shell *shell, double delta_seconds) {
  ...
  for (size_t index = 0; index < shell->dock_model.item_count; ++index) {
    if (shell->dock_item_x_animating[index]) {
      reach_float_animation_update(&shell->dock_item_x_animations[index],
                                   delta_seconds);
      shell->dock_item_x_animating[index] = reach_shell_float_animation_active(
          &shell->dock_item_x_animations[index]);
      shell->dock.dirty_flags = 1;
    }
  }
  if (shell->dock_drag.snapping) {
    reach_float_animation_update(&shell->dock_drag.snap_animation,
                                 delta_seconds);
  ...
  if (window_manager_dirty && shell->window_manager.ops.refresh != nullptr) {
    ...
    uintptr_t foreground_window =
        shell->window_manager.ops.foreground != nullptr
            ? shell->window_manager.ops.foreground(
                  shell->window_manager.manager)
            : 0;
  ...
  if (shell->launcher.window.ops.set_bounds != nullptr &&
      shell->monitors.list != nullptr && shell->monitors.ops.count != nullptr &&
    ...
    REACH_ASSERT(monitor->primary ||
                 shell->monitors.ops.count(shell->monitors.list) == 1);
    if (monitor == nullptr) {
      return REACH_ERROR;
    }
    ...
    if (shell->launcher.renderer.ops.begin_frame != nullptr) {
      ...
      if (reach_ui_layout_compute(&shell->ui, &input, &layout) == REACH_OK) {
        ...
        if (!game_mode) {
          reach_shell_apply_dock_width_animation(shell, &layout.dock,
                                                 delta_seconds);
        }
        reach_rect_f32 shown_dock_bounds = layout.dock.bounds;
        reach_rect_f32 animated_dock_bounds =
            game_mode ? shown_dock_bounds
                      : reach_shell_apply_dock_animation(
                            shell, shown_dock_bounds, bounds, delta_seconds);
        if (!game_mode) {
          ...
          shell->dock_reveal.check_dirty = 0;
          if (shell->dock_reveal_edge.ops.hide != nullptr) {
            (void)shell->dock_reveal_edge.ops.hide(
        ...
        float dock_right_offset =
            ...
            (layout.dock.bounds.x + layout.dock.bounds.width);
        ...
        int32_t dock_layout_changed =
            !shell->has_layout ||
            !reach_shell_rect_equal(shell->layout.dock.bounds,
                                    layout.dock.bounds);
        int32_t launcher_layout_changed =
            !shell->has_layout ||
            !reach_shell_rect_equal(shell->layout.launcher.bounds,
        ...
        if (shell->dock.window.ops.set_bounds != nullptr) {
          int32_t dock_window_changed = 0;
          float dock_radius = reach_theme_dock_corner_radius(
              shell->theme, layout.dock.bounds.height);
          result = reach_shell_apply_window_state(
        ...
        if (shell->tray.window.ops.set_bounds != nullptr) {
          ...
          if (shell->tray_state.popup_open) {
            ...
            if (shell->dirty.render || shell->tray.dirty_flags ||
                tray_window_changed) {
        ...
        if (shell->quick_settings.window.ops.set_bounds != nullptr) {
          ...
          if (quick_window_changed &&
              shell->quick_settings.window.ops.apply_rounded_corners !=
                  nullptr) {
        ...
        if (shell->switcher.window.ops.set_bounds != nullptr) {
          ...
          if (shell->switcher_state.open) {
            if (shell->switcher.window.ops.show != nullptr) {
              (void)shell->switcher.window.ops.show(
            ...
            (void)shell->switcher.window.ops.hide(
                shell->switcher.window.window);
          }
        }
        ...
        if (shell->context_menu.window.ops.set_bounds != nullptr) {
          ...
          result = reach_shell_apply_window_state(
              ...
              &shell->context_menu.opacity_valid, &context_window_changed);
          if (result != REACH_OK) {
            return result;
          }
...
int32_t reach_shell_needs_frame(const reach_shell *shell) {
  ...
  if (shell != nullptr) {
    ...
    if (shell->window_manager.manager != nullptr &&
        shell->window_manager.ops.needs_refresh != nullptr) {
      window_manager_needs_refresh = shell->window_manager.ops.needs_refresh(
          shell->window_manager.manager);
    }
  }

  return shell != nullptr &&
         (shell->dirty.update_requested || window_manager_needs_refresh ||
          shell->dirty.render || shell->dock.dirty_flags ||
          shell->launcher.dirty_flags || shell->tray.dirty_flags ||
          shell->switcher.dirty_flags || shell->context_menu.dirty_flags ||
