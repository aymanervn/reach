                                        ...
                                        uint32_t event_count)
{
    ...
    DWORD written = 0;
    if (!WriteFile(pipe, &event, sizeof(event), &written, nullptr) || written != sizeof(event))
    {
...
static LPARAM reach_helper_pack_event_types(const reach_ui_event_type *event_types,
                                            uint32_t event_count)
{
    uint32_t packed = 0;
    if (event_count > 0)
    {
        packed |= (uint32_t)event_types[0] & 0xffffu;
    }
    if (event_count > 1)
    {
        packed |= ((uint32_t)event_types[1] & 0xffffu) << 16;
    }
    return (LPARAM)packed;
}

static int32_t reach_helper_post_events(const reach_ui_event_type *event_types,
                                        uint32_t event_count)
{
    if (event_types == nullptr || event_count == 0 || event_count > 2 || g_hotkeys.thread_id == 0)
    {
        return 0;
    }

    return PostThreadMessageW(g_hotkeys.thread_id, REACH_HELPER_WM_HOTKEY_EVENTS, event_count,
                              reach_helper_pack_event_types(event_types, event_count)) != FALSE;
}

static int32_t reach_helper_post_event(reach_ui_event_type event_type)
{
    return reach_helper_post_events(&event_type, 1);
}

static void reach_helper_send_posted_events(WPARAM wparam, LPARAM lparam)
{
    uint32_t event_count = (uint32_t)wparam;
    uint32_t packed = (uint32_t)lparam;
    reach_ui_event_type event_types[2] = {};

    if (event_count == 0 || event_count > 2)
    {
        return;
    }

    event_types[0] = (reach_ui_event_type)(packed & 0xffffu);
    if (event_count > 1)
    {
        event_types[1] = (reach_ui_event_type)((packed >> 16) & 0xffffu);
    }

    if (!reach_helper_send_events(event_types, event_count))
    {
        reach_helper_disarm_hotkeys_after_channel_failure();
    }
}
...
static LRESULT CALLBACK reach_helper_keyboard_proc(int code, WPARAM wparam, LPARAM lparam)
{
    if (code == HC_ACTION)
    {
            ...
            reach_helper_hotkeys_armed())
        {
            ...
            if ((g_hotkeys.hotkey_mask & REACH_ELEVATION_HELPER_HOTKEY_ALT_TAB) != 0)
            {
                    ...
                    keyboard->vkCode == VK_RMENU)
                {
                    ...
                    else if (key_up)
                    {
                        ...
                        {
                            g_hotkeys.alt_tab_active = 0;
                            if (!reach_helper_post_event(REACH_UI_EVENT_ALT_TAB_COMMIT))
                            {
                                reach_helper_disarm_hotkeys_after_channel_failure();
                            }
                        }
                    ...
                    (g_hotkeys.alt_down || (keyboard->flags & LLKHF_ALTDOWN) != 0))
                {
                    ...
                    if (!g_hotkeys.alt_tab_active)
                    {
                        ...
                        sent = reach_helper_post_events(events, 2);
                    }
                    else
                    {
                        sent = reach_helper_post_event(direction);
                    }
                    if (sent)
                    {
                ...
                if (keyboard->vkCode == VK_ESCAPE && g_hotkeys.alt_tab_active && key_down)
                {
                    if (reach_helper_post_event(REACH_UI_EVENT_ALT_TAB_CANCEL))
                    {
                        g_hotkeys.alt_tab_active = 0;
                        return 1;
                    }
            ...
            if ((g_hotkeys.hotkey_mask & REACH_ELEVATION_HELPER_HOTKEY_WINDOWS_KEY) != 0)
            {
                if (keyboard->vkCode == VK_LWIN || keyboard->vkCode == VK_RWIN)
                {
                    ...
                    if (key_up)
                    {
                        ...
                        int32_t chord = g_hotkeys.windows_key_chord;
                        reach_helper_reset_windows_key_state();
                        if (!chord && !reach_helper_post_event(REACH_UI_EVENT_WINDOWS_KEY))
                        {
                            reach_helper_disarm_hotkeys_after_channel_failure();
                        }
                        return 1;
                ...
                if (g_hotkeys.windows_key_down && key_down)
                {
...
static DWORD WINAPI reach_helper_hotkey_thread(void *param)
{
    ...
    {
        if (message.message == REACH_HELPER_WM_HOTKEY_EVENTS)
        {
            reach_helper_send_posted_events(message.wParam, message.lParam);
        }
    }
...
static reach_result
reach_helper_set_hotkey_forwarding(const reach_elevation_helper_request *request)
{
    if (request == nullptr)
    {
        return REACH_INVALID_ARGUMENT;
...
static reach_result reach_helper_execute(const reach_elevation_helper_request *request)
{
    ...
    case REACH_ELEVATION_HELPER_COMMAND_SNAP:
        return reach_window_management_snap(hwnd,
