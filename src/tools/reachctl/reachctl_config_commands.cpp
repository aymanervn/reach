...
static int32_t reachctl_is_supported_wallpaper_path(const uint16_t *path)
{
    ...
    const wchar_t *extension =
        PathFindExtensionW(reinterpret_cast<const wchar_t *>(path));

    return lstrcmpiW(extension, L".jpg") == 0 ||
        lstrcmpiW(extension, L".jpeg") == 0 ||
        lstrcmpiW(extension, L".png") == 0 ||
...
int reachctl_wallpaper_monitor_command(
    ...
    const wchar_t *path)
{
    if (index_text == nullptr || index_text[0] == 0 ||
        path == nullptr || path[0] == 0) {
