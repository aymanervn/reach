...
static reach_result reachctl_elevation_helper_exe(uint16_t *path,
                                                  DWORD path_count)
{
...
reach_result reachctl_install_elevation_helper(void)
{
    ...
    const wchar_t *helper_path =
        reinterpret_cast<const wchar_t *>(helper_path_u16);
    ...
    swprintf_s(
        arguments,
        L"/Create /F /SC ONLOGON /TN \"%ls\" /TR \"\\\"%ls\\\"\" /RL HIGHEST",
        REACHCTL_HELPER_TASK_NAME,
        helper_path);
