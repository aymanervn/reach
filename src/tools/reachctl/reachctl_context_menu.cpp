...
static void reachctl_escape_powershell_single_quoted(
    wchar_t *dst,
    size_t dst_count,
    const wchar_t *src)
{
    ...
    const wchar_t *icon_path)
{
    ...
    reachctl_escape_powershell_single_quoted(
...
static reach_result reachctl_install_pin_context_menu(
    const wchar_t *reachctl_path,
    const wchar_t *icon_path)
{
...
static reach_result reachctl_install_wallpaper_monitor_context_menu(
    const wchar_t *reachctl_path,
    const wchar_t *icon_path)
{
...
static reach_result reachctl_remove_wallpaper_monitor_context_menu(void)
{
    ...
    for (size_t index = 0; index < sizeof(old_extensions) / sizeof(old_extensions[0]); ++index) {
        ...
        swprintf_s(
            key,
            L"Software\\Classes\\%ls\\shell\\ReachWallpaperMonitor",
...
reach_result reachctl_install_context_menus(
    const uint16_t *reachctl_path,
    const uint16_t *reach_icon_path)
{
    if (reachctl_path == nullptr || reachctl_path[0] == 0 ||
        reach_icon_path == nullptr || reach_icon_path[0] == 0) {
        return REACH_INVALID_ARGUMENT;
    }

    const wchar_t *reachctl_path_w =
        reinterpret_cast<const wchar_t *>(reachctl_path);
    const wchar_t *reach_icon_path_w =
        reinterpret_cast<const wchar_t *>(reach_icon_path);

    reach_result pin_result = reachctl_install_pin_context_menu(
        reachctl_path_w,
        reach_icon_path_w);
    ...
    return reachctl_install_wallpaper_monitor_context_menu(
        reachctl_path_w,
        reach_icon_path_w);
}
...
reach_result reachctl_remove_context_menus(void)
{
    ...
    return pin_result == REACH_OK && wallpaper_result == REACH_OK
        ? REACH_OK
        : REACH_ERROR;
}
