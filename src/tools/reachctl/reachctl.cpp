...
int wmain(int argc, wchar_t **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (lstrcmpiW(argv[index], L"--start") == 0) {
      ...
      reachctl_print(ok ? L"Reach started for current session."
    ...
    if (lstrcmpiW(argv[index], L"--reset") == 0) {
      ...
      reachctl_print(
          ok ? L"Reach reset complete. Windows shell restored."
    ...
    if (lstrcmpiW(argv[index], L"--wallpaper-monitor") == 0) {
      ...
      return reachctl_wallpaper_monitor_command(argv[index + 1],
                                                argv[index + 2]);
    }
  }
