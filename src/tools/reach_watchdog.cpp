...
static const wchar_t *REACHCTL_PROCESS_NAME = L"reachctl.exe";
...
static int reach_watchdog_sibling_path(
...
static int reach_watchdog_working_directory(
    const wchar_t *path,
...
static int reach_watchdog_start_process(
    const wchar_t *path,
...
static int reach_watchdog_open_process_by_name(
...
static void reach_watchdog_reset_with_reachctl_or_fallback(
    const wchar_t *reachctl_path,
    const wchar_t *working_directory)
{
    int started_reset = reach_watchdog_start_process(
        reachctl_path,
        L"--reset",
        working_directory);
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int)
{
    ...
    if (!reach_watchdog_sibling_path(REACH_PROCESS_NAME, reach_path, MAX_PATH) ||
        !reach_watchdog_sibling_path(REACHCTL_PROCESS_NAME, reachctl_path, MAX_PATH)) {
        reach_watchdog_reset_direct_fallback();
        return 1;
    }
    ...
    for (;;) {
        ...
        if (!reach_watchdog_open_process_by_name(REACH_PROCESS_NAME, &reach_process)) {
            if (!reach_watchdog_start_process_with_handle(
                ...
                if (failed_starts >= REACH_MAX_FAILED_STARTS) {
                    reach_watchdog_reset_with_reachctl_or_fallback(
                        reachctl_path,
                        working_directory[0] != 0 ? working_directory : nullptr);
                    return 1;
                }
        ...
        DWORD startup_wait = WaitForSingleObject(
            reach_process,
            REACH_STARTUP_GRACE_MS);
