...
static uint32_t reach_helper_hotkey_key_from_vk(DWORD vk)
{
    ...
    case VK_LMENU:
    case VK_RMENU:
        return REACH_SERVICE_HOTKEY_ALT;

    case VK_SHIFT:
    case VK_LSHIFT:
    case VK_RSHIFT:
        return REACH_SERVICE_HOTKEY_SHIFT;

    case VK_TAB:
        return REACH_SERVICE_HOTKEY_TAB;

    case VK_ESCAPE:
        return REACH_SERVICE_HOTKEY_ESCAPE;

    case VK_LWIN:
        return REACH_SERVICE_HOTKEY_LEFT_WIN;
...
static uint32_t reach_helper_hotkey_modifiers(void)
{
    ...
    {
        modifiers |= REACH_SERVICE_MODIFIER_ALT;
    }
    if (g_hotkeys.shift_down)
    {
        modifiers |= REACH_SERVICE_MODIFIER_SHIFT;
    }
    if (g_hotkeys.left_win_down)
    {
        modifiers |= REACH_SERVICE_MODIFIER_LEFT_WIN;
    }
    if (g_hotkeys.right_win_down)
    {
        modifiers |= REACH_SERVICE_MODIFIER_RIGHT_WIN;
    }
...
{
    (void)reach_service_shared_append_hotkey(key, action, modifiers);
}
...
{
    if (event->key == REACH_SERVICE_HOTKEY_TAB && event->down && g_hotkeys.alt_down)
    {
    ...
    if (event->key == REACH_SERVICE_HOTKEY_ALT && event->up && g_hotkeys.alt_tab_active)
    {
        reach_helper_append_hotkey(event->key, REACH_SERVICE_HOTKEY_RELEASED,
                                   reach_helper_hotkey_modifiers());
        g_hotkeys.alt_tab_active = 0;
...
reach_helper_handle_windows_key(const reach_helper_key_event *event)
{
    ...
    if (event->up)
    {
        ...
        {
            modifiers |= REACH_SERVICE_MODIFIER_CHORD;
        }

        reach_helper_append_hotkey(event->key, REACH_SERVICE_HOTKEY_RELEASED, modifiers);
