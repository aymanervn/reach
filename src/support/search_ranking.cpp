...
static int32_t reach_search_extension_in(const uint16_t *extension, const char *const *values, size_t count)
{
...
reach_search_result_kind reach_search_classify_result(const uint16_t *path, int32_t is_directory)
{
    ...
    static const char *const photo_extensions[] = { ".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp", ".tif", ".tiff" };
    static const char *const video_extensions[] = { ".mp4", ".mkv", ".mov", ".avi", ".webm", ".wmv" };
    static const char *const music_extensions[] = { ".mp3", ".wav", ".flac", ".aac", ".ogg", ".m4a", ".wma" };
    static const char *const document_extensions[] = { ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".txt", ".rtf", ".md" };

    const uint16_t *extension = reach_search_extension(path);
    if (reach_search_extension_in(extension, app_extensions, sizeof(app_extensions) / sizeof(app_extensions[0]))) {
        return REACH_SEARCH_RESULT_APP;
    }
    if (reach_search_extension_in(extension, photo_extensions, sizeof(photo_extensions) / sizeof(photo_extensions[0]))) {
        return REACH_SEARCH_RESULT_PHOTO;
    }
    if (reach_search_extension_in(extension, video_extensions, sizeof(video_extensions) / sizeof(video_extensions[0]))) {
        return REACH_SEARCH_RESULT_VIDEO;
    }
    if (reach_search_extension_in(extension, music_extensions, sizeof(music_extensions) / sizeof(music_extensions[0]))) {
        return REACH_SEARCH_RESULT_MUSIC;
    }
    if (reach_search_extension_in(extension, document_extensions, sizeof(document_extensions) / sizeof(document_extensions[0]))) {
...
int32_t reach_search_score_candidate(const uint16_t *query, const reach_search_candidate *candidate)
{
    ...
    const uint16_t *name = candidate->name[0] != 0 ? candidate->name : reach_search_basename(candidate->path);
    ...
    if (app && (reach_search_equals_ci(name, query) || reach_search_stem_equals_ci(name, query))) {
        ...
        score = 600;
    } else if (!app && (reach_search_equals_ci(name, query) || reach_search_stem_equals_ci(name, query))) {
        score = 500;
    } else if (!app && reach_search_starts_with_ci(name, query)) {
        score = 400;
    } else if (!app && reach_search_contains_ci(name, query)) {
...
size_t reach_search_rank_candidates(
    const uint16_t *query,
    reach_search_candidate *candidates,
