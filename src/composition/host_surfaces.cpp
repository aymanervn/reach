...
/*
 * Item 7 surface taxonomy: the descriptor table + class-driven policy queries.
 * is_open reads capsule state through queries; force_close routes through the
 * same per-surface close functions the translators use (their cross-feature
 * extras — focus policy, worker cancel — live in those functions).
 */
...
void reach_host_init_surface_descriptors(reach_host *host)
{
    ...
    /* Pointer dispatch registration: the window-event source tag, the loop
       precedence (lower dispatches first; popups < transients < persistent —
       the order the hand-written chains used), the per-feature action
       translator, and the outside-press dismissal where it differs from
       force_close by focus policy. */
    ...
    /* Frame steps, in dependency order (the dock cluster's layout resolves
       before the loop; the launcher renders first, the context menu last so
       it stacks above every anchor). */
    ...
    /* Level-3 activation: features declare which events open them; composition
       supplies the host-level toggle/route hooks (they run the cross-feature
       open policy). The generic loops live in host_input.cpp. */
...
/*
 * Class rule: popups are mutually exclusive — opening one closes the others
 * through their descriptor close hooks. A new popup-class surface inherits
 * this without composition changes.
 */
