...
/* The clock text formatting + minute guard live in the dock capsule
 * (reach_dock_update_clock); composition only routes its redraw result onto the
 * dock surface's dirty flag. */
...
static void reach_host_tick_animations(reach_host *host, double delta_seconds)
{
    ...
    /* Class loop: every capsule ticks through its uniform hook; the reported
       effects route onto the surface dirty flags. Feature-specific settle
       logic lives inside the capsules' ticks now. */
...
reach_result reach_host_update(reach_host *host, double delta_seconds)
{
    ...
    /* Re-assert the popup mouse hook periodically: the OS silently drops a
       low-level hook whose thread stalled, which would leave open popups
       unable to close on outside clicks. No-op while nothing is open. */
    ...
    if (window_manager_dirty && host->window_manager.ops.refresh != nullptr)
    {
        ...
        /* NAMED EXCEPTION (not a class rule): the launcher closes when the
           foreground window changes — focus moved to a real app, the overlay
           yields. If more per-feature policies like this accumulate, that is
           the signal a class rule is missing. */
        ...
        host->monitors.ops.count(host->monitors.list) > 0)
    {
        ...
        if (host->launcher.renderer.ops.begin_frame != nullptr)
        {
            ...
            if (dock_layout_result == REACH_OK && launcher_layout_result == REACH_OK)
            {
                ...
                /* Slot widths animate inside build_layout/rebuild_items; while
                   they move the dock keeps redrawing. */
                ...
                /*
                    Per-surface frame steps (host_surface_frames.cpp), as a
                    loop over the descriptor table in frame_priority order:
                    the dock cluster resolved above; each step applies layout
                    refresh -> transition -> window state -> corners ->
                    show/render.
                */
...
int32_t reach_host_needs_frame(const reach_host *host)
{
    ...
    /* Composition-owned frame reasons: dirty state, the async workers, the
       host animation manager, and the window-manager refresh signal. */
    ...
    /* Class loop: every capsule reports its own pending animations/work. */
    for (size_t index = 0; index < REACH_HOST_SURFACE_COUNT; ++index)
    {
        const reach_surface_desc *desc = &host->surface_descs[index];
