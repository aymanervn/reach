...
{
    /* Class-driven: any open transient or popup surface holds the dock revealed. */
...
void reach_host_sync_pointer_move_subscriptions(reach_host *host)
{
    ...
    /* Class loop: each capsule reports whether a drag/hover capture wants
       pointer-move events on its surface. The dock adds the composition-owned
       auto-hide policy (can-hide keeps its surface subscribed). */
                                                     ...
                                                     reach_rect_f32 monitor_bounds)
{
    ...
    /*
        Gather the cross-component policy inputs, let the dock capsule run the
        reveal state machine + Y animation, then apply the result through the ports
        composition owns (reveal-edge surface, pointer-move subscriptions, sticky
        feedback). The decision arithmetic itself lives in
        reach_dock_update_visibility (moved verbatim).
    */
...
{
    /* Config distribution: composition assembles the borrowed inputs the dock's
       build/rebuild ops consume (theme, dpi, ui config, pinned apps). */
