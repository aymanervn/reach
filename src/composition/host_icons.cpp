...
/*
 * Render-icon lifetime. App/window icon LOADING + caching live entirely in
 * the icon service (pull model: features get(path, size) at render time);
 * composition keeps only the render-side eviction — an icon image may be
 * cached by whichever surface drew it, so eviction stays a host policy — and
 * the release of adapter-produced render ids (tray items, quick-settings
 * sessions/devices, and the Now Playing cover).
 */
