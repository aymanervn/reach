...
reach_host_dispatch_pointer(reach_host *host, reach_surface_id surface_id,
...
/* --- Generic dispatch plumbing over the descriptor table --- */
...
/* Dispatch precedence: lower pointer_priority first (popups < transients <
   persistent — the order the retired hand-written chains used). */
                                       ...
                                       reach_surface_id out[REACH_HOST_SURFACE_COUNT])
{
    ...
    for (size_t index = 0; index < REACH_HOST_SURFACE_COUNT; ++index)
    {
        ...
        size_t at = count;
        while (at > 0 &&
               host->surface_descs[out[at - 1]].pointer_priority > desc->pointer_priority)
        {
            out[at] = out[at - 1];
            --at;
        }
...
/* Open gate for the generic loops: a surface without an is_open hook
   (persistent) is always reachable; popups/transients only while open. */
...
static reach_result reach_host_handle_pointer_wheel(reach_host *host,
                                                     const reach_ui_event *event)
{
    ...
    /* Class loop: open surfaces in dispatch order; the first that handles the
       wheel (or reports an action) wins. */
    ...
    for (size_t index = 0; index < order_count; ++index)
    {
        ...
        reach_capsule_pointer_result wheel = reach_host_dispatch_pointer(
            host, desc->id, event, REACH_POINTER_EVENT_WHEEL);
...
reach_result reach_host_apply_dock_pointer_action(
    reach_host *host, const reach_ui_event *event, const reach_capsule_pointer_result *result)
{
    if (host == nullptr || result == nullptr)
    {
    ...
    case REACH_DOCK_POINTER_ACTION_SHOW_ITEM_CONTEXT:
    {
        ...
        reach_result show_result = reach_host_show_dock_app_context_menu(
            host, result->action.index, event->x, event->y);
...
/* A pointer-up consumed by another surface must not leave a dock press
   sequence armed (stuck feedback / capture); cancel it through the same ops. */
                                                  ...
                                                  reach_surface_role source)
{
    ...
    /* Capture pre-emption (named): an active dock drag, QS slider drag, or
       launcher scrollbar drag owns the release regardless of source surface.
       (wants_pointer_move doubles as hover intent for some capsules, so the
       capture set stays explicit.) */
    if (reach_dock_state_ptr(host->dock_capsule)->drag.active)
    {
        reach_capsule_pointer_result dock_drag = reach_host_dispatch_pointer(
            host, REACH_SURFACE_ID_DOCK, event, REACH_POINTER_EVENT_UP);
    ...
    /* Class loop: open popup/transient surfaces in dispatch order; the dock
       (persistent) keeps its source/sequence tail below. */
    reach_surface_id order[REACH_HOST_SURFACE_COUNT];
    size_t order_count = reach_host_pointer_order(host, order);
    for (size_t index = 0; index < order_count; ++index)
    {
    ...
    {
        reach_capsule_pointer_result dock_up = reach_host_dispatch_pointer(
            host, REACH_SURFACE_ID_DOCK, event, REACH_POINTER_EVENT_UP);
        if (dock_up.handled || dock_up.action.kind != REACH_DOCK_POINTER_ACTION_NONE)
        {
            /* Named popup rule: launching/focusing from the dock closes an
               open tray popup unless the press landed inside it. */
                                                    ...
                                                    reach_surface_role source)
{
    ...
    if (reach_quick_settings_state_ptr(host->quick_settings_capsule)->open)
    {
        ...
        if (reach_rect_contains(reach_quick_settings_state_ptr(host->quick_settings_capsule)->bounds, event->x, event->y))
        {
    ...
    /* Class loop: remaining open surfaces in dispatch order (source-gated
       surfaces were already offered the press above). An unhandled press runs
       the surface's declared outside-press policy. */
    reach_surface_id order[REACH_HOST_SURFACE_COUNT];
    size_t order_count = reach_host_pointer_order(host, order);
    for (size_t index = 0; index < order_count; ++index)
    {
        ...
        {
            /* Outside press dismisses the surface (the launcher's dismiss
               keeps focus with a hit dock, otherwise restores). */
                                                    ...
                                                    reach_surface_role source)
{
    ...
    /* Capture pre-emption (named): an active QS slider drag owns the stream
       ahead of any hover consumer. */
    const reach_surface_desc *quick_settings_desc =
        &host->surface_descs[REACH_SURFACE_ID_QUICK_SETTINGS];
    if (quick_settings_desc->capsule_ops->wants_pointer_move != nullptr &&
        quick_settings_desc->capsule_ops->wants_pointer_move(quick_settings_desc->capsule))
    ...
    /* Class loop: a surface sees moves from its own window, or from anywhere
       while its capsule wants the move stream (drag capture / hover intent). */
    reach_surface_id order[REACH_HOST_SURFACE_COUNT];
    size_t order_count = reach_host_pointer_order(host, order);
    for (size_t index = 0; index < order_count; ++index)
    {
...
static reach_result reach_host_handle_pointer_middle(reach_host *host,
                                                      const reach_ui_event *event,
                                                      reach_surface_role source)
{
    if (host == nullptr || event == nullptr || !host->has_layout)
    ...
    /* Named exception: the open tray popup absorbs middle clicks from any
       surface (middle-click closes the item window). */
    reach_capsule_pointer_result tray_middle = reach_host_dispatch_pointer(
        host, REACH_SURFACE_ID_TRAY, event, REACH_POINTER_EVENT_MIDDLE);
    (void)reach_host_apply_tray_pointer_action(host, event, &tray_middle);

    /* Generic: the source surface receives its own middle clicks. */
...
static reach_result reach_host_handle_pointer_leave(reach_host *host, reach_surface_role source)
{
    ...
    /* Generic: the source surface owns its leave semantics. */
    const reach_surface_desc *src = reach_host_surface_for_role(host, source);
    if (src != nullptr && src->capsule_ops->handle_pointer != nullptr)
    {
        ...
        {
            /* Auto-hide policy re-evaluates when the pointer leaves the dock. */
            reach_host_request_dock_visibility_update(host);
        }
    }
...
static reach_result reach_host_handle_pointer_cancel(reach_host *host)
{
    ...
    /* Class loop: every pointer-capable capsule resets its interaction. */
    reach_surface_id order[REACH_HOST_SURFACE_COUNT];
    size_t order_count = reach_host_pointer_order(host, order);
    for (size_t index = 0; index < order_count; ++index)
    {
        reach_capsule_pointer_result cancel = reach_host_dispatch_pointer(
            host, order[index], nullptr, REACH_POINTER_EVENT_CANCEL);
...
static reach_result reach_host_handle_pointer_context(reach_host *host,
                                                       const reach_ui_event *event,
                                                       reach_surface_role source)
{
    if (host == nullptr || event == nullptr || !host->has_layout)
    ...
    /* An open context menu sees the right-click first (dismiss semantics);
       flow continues so the click can still open a new menu elsewhere. */
    if (reach_context_menu_is_open(host->context_menu_capsule))
    {
    ...
    /* Named popup rule: a right-click anywhere closes the QS popup. */
    if (reach_quick_settings_state_ptr(host->quick_settings_capsule)->open)
    {
        reach_host_set_quick_settings_open(host, 0);
    }

    /* Class loop: open surfaces in dispatch order (source-gated surfaces only
       from their own window); first handled/action result wins. */
    reach_surface_id order[REACH_HOST_SURFACE_COUNT];
    size_t order_count = reach_host_pointer_order(host, order);
    for (size_t index = 0; index < order_count; ++index)
    {
...
static reach_result reach_host_handle_surface_event(reach_host *host,
                                                     const reach_ui_event *event,
                                                     reach_surface_role source);
                                                     ...
                                                     reach_surface_role source)
{
    ...
    if (event->type == REACH_UI_EVENT_LAUNCHER_SEARCH_READY)
    {
        ...
        /* The generic wake also follows icon-load completions: redraw the
           icon-consuming surfaces so the next pull hits the cache (placeholders
           would otherwise linger until an unrelated repaint). */
    ...
    /* Level-3 activation: modal event streams go whole to their declared
       owner (the switcher's alt-tab family today). */
    for (size_t index = 0; index < REACH_HOST_SURFACE_COUNT; ++index)
    {
        const reach_surface_desc *desc = &host->surface_descs[index];
    ...
    {
        /* The launcher owns text input end-to-end (edit model, query, search);
           composition applies the reported surface effects. */
    ...
    /* Level-3 activation: toggle every surface that declared this event (the
       windows key opens the launcher AND the clipboard — two declarations, no
       special case). The launcher's open/close surface effects apply through
       the open-state diff below. */
    for (size_t index = 0; index < REACH_HOST_SURFACE_COUNT; ++index)
    {
        const reach_surface_desc *desc = &host->surface_descs[index];
    ...
    /* NAMED EXCEPTION (dismissal, not activation): Escape closes the clipboard;
       the launcher handles its own Escape in-surface. */
    ...
    {
        return reach_host_open_pinned_app_id(host, intent.id, 0, reach_launcher_is_open(host->launcher_capsule));
    }
