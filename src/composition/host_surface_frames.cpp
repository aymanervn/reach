...
/*
 * Per-surface frame step (Phase-2 layout work). reach_host_update resolves
 * layout in dependency order (monitor -> dock cluster -> launcher ->
 * clipboard -> switcher) and then applies each surface's frame here:
 * transition bounds + opacity -> window state -> rounded corners ->
 * show/render or hide. The shared mechanics live in
 * reach_host_apply_transient_frame; the launcher (focus policy, no corner
 * radius) and the dock (persistent, no transition, reveal fast-path) keep
 * bespoke frames. The bodies are verbatim relocations from reach_host_update.
 */

typedef struct reach_host_frame_state
{
    int32_t window_changed;
    int32_t visible; /* shown this frame: not game mode + transition visible */
...
reach_result reach_host_frame_launcher(reach_host *host, const reach_host_frame_context *ctx)
{
    ...
    {
        /*
            The launcher surface owns keyboard focus directly now (no
            native EDIT child window), so showing it once on open both
            reveals and focuses it; the focus guard avoids re-stealing
            focus every frame while the launcher stays open.
        */
...
reach_result reach_host_frame_dock(reach_host *host, const reach_host_frame_context *ctx)
{
    ...
    /* Reveal fast-path: while only the Y reveal animation moves the dock, the
       window slides without a re-render. */
...
reach_result reach_host_frame_switcher(reach_host *host, const reach_host_frame_context *ctx)
{
    ...
    {
        /* The switcher renders at its transitioned bounds (it slides with the
           show/hide transition, unlike the other popups). */
                                           ...
                                           const reach_host_frame_context *ctx)
{
    ...
    if (!game_mode)
    {
        /* Follow the dock: the anchor button slides while slots animate, so an
           open menu re-places itself each frame (tray and QS already do). */
