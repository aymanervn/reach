...
/*
 * Surface taxonomy (encapsulation plan, Item 7). Every top-level surface
 * registers a descriptor; the host's lifecycle/visibility policy (which
 * surfaces count as "transient open", what an outside click closes, the mouse
 * hook, dirty/transition sweeps) runs over classes instead of naming features.
 * Now Playing is a private part of the dock capsule; the reveal-edge helper
 * stays a dock implementation detail.
 */
typedef enum reach_surface_class
{
    REACH_SURFACE_CLASS_PERSISTENT = 0, /* dock — always-on, auto-hide policy */
    REACH_SURFACE_CLASS_TRANSIENT = 1,  /* launcher, clipboard — outside click closes */
    REACH_SURFACE_CLASS_POPUP = 2,      /* tray, quick settings, context/power menu */
    REACH_SURFACE_CLASS_OVERLAY = 3     /* switcher — held open by the alt-tab gesture */
} reach_surface_class;
...
typedef enum reach_surface_pointer_flags
{
    ...
    /* DOWN/MOVE/CONTEXT reach this surface only from its own window (its
       capsule still sees UP/WHEEL globally). */
    REACH_SURFACE_POINTER_SOURCE_GATED = 1u << 2,
    /* Unhandled DOWN while open dismisses the surface (outside-press close);
       runs `dismiss` (which may differ from force_close by focus policy). */
    REACH_SURFACE_POINTER_DOWN_CLOSES_ON_UNHANDLED = 1u << 3,
    /* Unhandled DOWN while open still applies the result — the capsule owns
       its own outside-press policy through actions. */
...
typedef struct reach_surface_desc
{
    ...
    reach_host_surface_transition *transition; /* null: no show/hide transition track */
    /* Close hook for the generic dismissal policy; null for classes the policy
       never force-closes (persistent, overlay). It runs the surface's full
       cross-feature close (transitions, hooks, focus policy), unlike the
       capsule ops' force_close which flips only feature state. */
    void (*force_close)(reach_host *host);
    /* The feature capsule behind this surface + its uniform hooks; the class
       loops (tick, needs_frame, game mode, reset, pointer-move sync, is-open)
       run over these. */
    void *capsule;
    const reach_feature_capsule_ops *capsule_ops;
    /* Declarative exceptions applied by the generic pointer dispatcher. */
    uint32_t pointer_flags;
    /* Window-event source tag; the pointer dispatcher resolves an event's
       source surface through this. */
    reach_surface_role role;
    /* Precedence in the generic pointer loops (lower dispatches first;
       popups < transients < persistent). */
    int32_t pointer_priority;
    /* Per-feature action translator (the mutations rule): the dispatcher
       applies handled results through this hook. */
    reach_result (*apply_pointer_action)(reach_host *host, const reach_ui_event *event,
                                         const reach_capsule_pointer_result *result);
    /* Outside-press dismissal for DOWN_CLOSES_ON_UNHANDLED; falls back to
       force_close when null. */
    void (*dismiss)(reach_host *host);
    /* Per-frame surface step (layout refresh -> transition -> window state ->
       corners -> show/render) and its position in the dependency-ordered
       frame pass (lower runs first; the dock cluster resolves before the
       loop). Null: the surface has no frame step. */
    reach_result (*frame)(reach_host *host, const struct reach_host_frame_context *ctx);
    int32_t frame_priority;
    /* Level-3 activation (feature-declared): events that toggle this surface
       open/closed, applied by the generic activation loop through the host
       `toggle` hook (which owns the surface's cross-feature open policy). */
    const reach_ui_event_type *toggle_events;
    size_t toggle_event_count;
    void (*toggle)(reach_host *host);
    /* Modal event stream (feature-declared) delivered whole to its owner
       (e.g. the switcher's APP_SWITCH_* family). */
...
/* Populate the table (host_surfaces.cpp); called once after the host exists. */
void reach_host_init_surface_descriptors(reach_host *host);

/* Class-driven policy queries over the table. */
...
/* Class rule: opening a popup closes the other popups. */
void reach_host_close_other_popups(reach_host *host, reach_surface_id keep);

/* Per-surface frame step (host_surface_frames.cpp): layout refresh ->
   transition -> window state -> rounded corners -> show/render or hide.
   reach_host_update runs the steps as a loop over the descriptor table in
   frame_priority order once the dock cluster resolves; the shared context
   carries the per-pass inputs every step may need. */
...
/* Cross-feature policy: a launcher-result launch waits for the launcher's
   close transition to finish before the app starts (the service only runs
   launches). */
...
/*
 * Composition-side dock reveal state: only the reveal-edge SURFACE state and the
 * per-surface pointer-move subscription flags. The reveal state machine itself
 * (target_hidden / reveal_session_active / pointer_sequence_active) + the Y-anim
 * init flag moved into the dock capsule (reach_dock_state) in F.
 */
typedef struct reach_host_dock_reveal_state
{
    ...
    int32_t move_enabled[7]; /* REACH_HOST_SURFACE_COUNT, indexed by surface id */
...
struct reach_host
{
    ...
    /* Dock configuration (from the config store; the old reach_ui_state wrapper
       around it is gone). */
    reach_dock_model dock_config;
    /* Composition-owned shared app data; fed into the dock/launcher layout +
       render as input. */
    ...
    /* Item 7: the surface-descriptor table lifecycle policy loops over. */
    ...
    /* Shared window state (snapshots, focus history, control worker) lives in
       the window-tracking service; composition reads through the accessors
       below and translates refresh reports into dock/icon effects. */
    ...
    /* Periodic popup-hook re-assert accumulator (see host_update). */
...
/* Window-tracking reads (the service owns the data). */
...
/* Level-3 activation toggle hook: remembers the focus-restore window when
   opening, then flips the capsule; the open/close surface effects apply via
   the launcher open-state diff in the event tail (host_input.cpp). */
...
/* Icon-service maintenance (main thread): recency trim + render eviction of
   the ids the service let go of. */
...
/* System-action translators (hotkeys + dock/QS actions -> port calls). */
...
/* Pointer-action translators living in host_input.cpp (registered on the
   surface descriptors; the mutations rule keeps them composition-side). */
