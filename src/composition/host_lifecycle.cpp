...
{
    /* The old worker posted CONFIG_CHANGED to the launcher window (falling
       back to a plain update request); keep that wake path. */
    reach_host *host = static_cast<reach_host *>(user);
    if (host == nullptr)
    {
        return;
...
{
    /* Wake the loop; the ready event's handler + the update pass drain the
       completed loads on the main thread. */
    reach_host_notify_launcher_search_ready(static_cast<reach_host *>(user));
}
...
static void reach_host_cleanup(reach_host *host)
{
    ...
    /*
     * The feature models used to be re-initialized here; they now live in the
     * feature capsules, which were already destroyed above. Re-initializing them
     * through destroyed (null) capsules right before `delete host` is both
     * meaningless and a null dereference, so it is intentionally omitted.
     */
}
                                                  ...
                                                  reach_host **out_shell)
{
    ...
    /*
     * Create the feature capsules first: the init/reset code below reaches each
     * feature's relocated state through its capsule accessor, so the capsules must
     * exist before any of those accessors run.
     */
    host->switcher_capsule = nullptr;
    ...
    /* Class loop: every top-level capsule resets its owned state. */
...
void reach_host_set_initial_foreground(reach_host *host, uintptr_t window)
{
    ...
    /* At startup the window list is still empty, so this is a plain seed (the
       focus history only tracks known open windows). */
