...
static void reach_host_handle_global_mouse_down(reach_host *host, reach_point_i32 point)
{
    ...
    /* While a surface's open transition still runs, its on-screen bounds are
       the mid-slide ones — an outside test against them can misclassify a
       click near the settling position and close the popup it just opened.
       Suppress outside-click closes for surfaces still opening. */
...
void reach_host_sync_popup_mouse_hook(reach_host *host)
{
    ...
    {
        /* Hook while any transient or popup surface is open (class-driven). */
...
void reach_host_close_transient_surfaces(reach_host *host, int32_t restore_launcher_focus)
{
    ...
    /* Close every transient/popup surface through its descriptor hook. The
       launcher is the one policy special case: the caller decides whether its
       close restores focus to the previous window. */
