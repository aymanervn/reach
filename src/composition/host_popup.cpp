...
static void reach_host_handle_global_mouse_down(reach_host *host, reach_point_i32 point)
{
    ...
    int32_t on_tray = reach_tray_state_ptr(host->tray_capsule)->popup_open &&
                      (float)point.x >= host->tray.last_bounds.x &&
    ...
    reach_rect_f32 context_bounds =
        host->context_menu.bounds_valid
            ? host->context_menu.last_bounds
            : reach_context_menu_state_ptr(host->context_menu_capsule)->bounds;
    int32_t on_context = reach_context_menu_state_ptr(host->context_menu_capsule)->open &&
    ...
    reach_rect_f32 quick_settings_bounds =
        host->quick_settings.bounds_valid
    ...
    int32_t on_quick_settings =
        ...
        (float)point.x >= quick_settings_bounds.x &&
    ...
    int32_t on_launcher = reach_launcher_is_open(host->launcher_capsule) &&
                          (float)point.x >= launcher_bounds.x &&
    ...
    reach_rect_f32 clipboard_bounds =
        host->clipboard_surface.bounds_valid
            ? host->clipboard_surface.last_bounds
            : reach_clipboard_feature_state_ptr(host->clipboard_capsule)->layout.bounds;
    int32_t on_clipboard = reach_clipboard_is_open(host->clipboard_capsule) &&
                           (float)point.x >= clipboard_bounds.x &&
    ...
    int32_t on_tray_button = reach_tray_state_ptr(host->tray_capsule)->popup_open &&
    ...
    int32_t on_power_button =
        reach_context_menu_state_ptr(host->context_menu_capsule)->open &&
    ...
    { return transition->target_open && reach_host_surface_transition_active(host, transition); };
    ...
    if (reach_context_menu_state_ptr(host->context_menu_capsule)->open && !on_context &&
        !on_power_button && !opening(&host->context_menu_transition))
    {
        reach_host_close_context_menu(host);
        reach_host_clear_sticky_dock_feedback(host);
    }
    if (reach_quick_settings_state_ptr(host->quick_settings_capsule)->open && !on_quick_settings &&
        !on_quick_settings_button && !opening(&host->quick_settings_transition))
    {
