...
/* The dock item's allowed commands come from the dock capsule (it owns the
   item capabilities); this checks a chosen command is still in that set. */
...
void reach_host_reanchor_context_menu(reach_host *host)
{
    ...
    {
        /* The anchored item left the dock; keep the last placement. */
        return;
    }
