...
static reach_rect_f32 reach_quick_settings_rect(float x, float y, float width,
                                                float height) {
  reach_rect_f32 rect = {};
  rect.x = x;
  rect.y = y;
...
static reach_rect_f32 reach_quick_settings_content_line(
    reach_rect_f32 content_bounds, float y, float height) {
  return reach_quick_settings_rect(
      content_bounds.x + reach_quick_settings_metrics_values.content_padding, y,
      content_bounds.width -
          reach_quick_settings_metrics_values.content_padding * 2.0f,
      height);
}
...
static float reach_quick_settings_stacked_rows_height(size_t row_count,
                                                      float row_height,
                                                      float row_gap) {
  if (row_count == 0) {
    return 0.0f;
...
void reach_quick_settings_model_set_main_volume(
    reach_quick_settings_model *model, float volume_level, int32_t muted) {
  if (model == nullptr) {
    return;
  }
...
uint32_t reach_quick_settings_volume_icon_id(float volume_level,
                                             int32_t muted) {
...
void reach_quick_settings_model_set_output_devices(
  ...
  model->output_devices.count =
      devices->count < REACH_AUDIO_VOLUME_MAX_OUTPUT_DEVICES
          ? devices->count
          : REACH_AUDIO_VOLUME_MAX_OUTPUT_DEVICES;
...
void reach_quick_settings_model_set_system_states(
    reach_quick_settings_model *model, const reach_network_state *network,
  ...
  model->bluetooth =
      bluetooth != nullptr ? *bluetooth : reach_bluetooth_state{};
  model->power = power != nullptr ? *power : reach_power_state{};
  model->brightness =
      brightness != nullptr ? *brightness : reach_brightness_state{};
  ...
  model->brightness.level =
      reach_quick_settings_clamp01(model->brightness.level);
...
void reach_quick_settings_model_set_bluetooth_pending(
    reach_quick_settings_model *model, int32_t pending,
...
void reach_quick_settings_volume_pill_model_init(
    reach_quick_settings_volume_pill_model *model, float volume_level,
    int32_t muted, const uint16_t *label) {
  if (model == nullptr) {
    return;
  }
  ...
  model->icon_id =
      reach_quick_settings_volume_icon_id(model->volume_level, model->muted);
  model->session_instance_id[0] = 0;
  reach_quick_settings_copy_utf16(
      model->label, REACH_AUDIO_VOLUME_SESSION_LABEL_CAPACITY, label);
}
...
reach_quick_settings_volume_pill_layout_for_bounds(reach_rect_f32 bounds,
  ...
  const reach_quick_settings_metrics &metrics =
      reach_quick_settings_metrics_values;

  layout.header_icon.x = bounds.x;
  layout.header_icon.y =
  ...
  layout.header_label = reach_quick_settings_rect(
      layout.header_icon.x + metrics.pill_header_icon_size +
          metrics.pill_header_label_gap,
      bounds.y - metrics.pill_header_gap - metrics.pill_header_height,
      bounds.width - metrics.pill_header_icon_size -
          metrics.pill_header_label_gap,
...
float reach_quick_settings_content_height_for_model(
  ...
  size_t visible_output_devices =
      reach_quick_settings_visible_output_device_count(model);
  ...
  float grid_component_height = reach_quick_settings_stacked_rows_height(
                                    grid_rows, metrics.system_grid_tile_height,
                                    metrics.system_grid_gap) +
                                metrics.system_grid_bottom_gap;
  float volume_component_height =
      metrics.pill_header_height + metrics.section_header_gap +
      metrics.pill_height;
  float brightness_component_height = 0.0f;
  if (model != nullptr && model->brightness.available) {
    brightness_component_height = metrics.pill_header_height +
  ...
  if (model != nullptr && model->output_devices_expanded) {
    output_component_height =
        metrics.output_button_gap + metrics.output_title_height +
  ...
  if (model != nullptr && model->expanded) {
    app_volume_component_height =
        metrics.app_title_gap + metrics.app_title_height +
  ...
  return metrics.content_padding * 2.0f + grid_component_height +
         brightness_component_height + volume_component_height +
         output_component_height + app_volume_component_height;
}
...
float reach_quick_settings_volume_pill_level_for_x(
  ...
  return reach_quick_settings_clamp01((x - layout->slider_track.x) /
                                      layout->slider_track.width);
}

static void reach_quick_settings_place_system_tile(
    reach_quick_settings_tile_layout *tile, reach_rect_f32 grid_bounds,
    float tile_width, size_t tile_index) {
  if (tile == nullptr) {
    return;
  }
...
reach_quick_settings_layout reach_quick_settings_layout_for_content_bounds(
  ...
  float tile_width = reach_quick_settings_clamp_min0(
      (grid_bounds.width - metrics.system_grid_gap) * 0.5f);

  size_t tile_index = 0;
  reach_quick_settings_place_system_tile(&layout.network_tile, grid_bounds,
                                         tile_width, tile_index++);
  reach_quick_settings_place_system_tile(&layout.bluetooth_tile, grid_bounds,
                                         tile_width, tile_index++);
  if (model != nullptr && model->power.has_battery) {
    reach_quick_settings_place_system_tile(&layout.battery_saver_tile,
                                           grid_bounds, tile_width,
                                           tile_index++);
    reach_quick_settings_place_system_tile(&layout.project_tile, grid_bounds,
                                           tile_width, tile_index++);
  }
  ...
  float next_y = layout.system_grid_bounds.y +
  ...
  if (model != nullptr && model->brightness.available) {
    reach_rect_f32 brightness_bounds = reach_quick_settings_content_line(
        content_bounds,
        next_y + metrics.pill_header_height + metrics.section_header_gap,
        metrics.pill_height);
    layout.brightness_pill = reach_quick_settings_volume_pill_layout_for_bounds(
        brightness_bounds, theme);
    ...
    next_y = brightness_bounds.y + brightness_bounds.height +
             metrics.system_grid_bottom_gap;
  }
  ...
  if (model != nullptr && model->output_devices_expanded) {
    ...
    layout.output_devices_title_chevron.y =
        ...
        (layout.output_devices_title.height - metrics.chevron_icon_size) *
            0.5f;
    ...
    for (size_t index = 0; index < visible_output_devices; ++index) {
      ...
      row->bounds.y = layout.output_devices_panel.y +
                      (float)index * metrics.output_device_row_height;
      ...
      row->icon.y =
          row->bounds.y +
          (row->bounds.height - metrics.output_icon_size) * 0.5f;
      ...
      row->checkmark.y =
          row->bounds.y +
          (row->bounds.height - metrics.output_check_size) * 0.5f;

      row->label.x = row->icon.x + metrics.output_icon_size +
                     metrics.output_row_label_gap;
      row->label.y = row->bounds.y;
      row->label.width =
          row->checkmark.x - metrics.output_row_label_gap - row->label.x;
      ...
      row->separator.y =
          row->bounds.y + row->bounds.height - metrics.separator_thickness;
      row->separator.width =
          row->bounds.width - metrics.separator_inset * 2.0f;
    ...
    layout.output_device_button = reach_quick_settings_content_line(
        content_bounds, next_y + metrics.output_button_gap,
    ...
    layout.output_device_button_chevron.x = layout.output_device_button.x +
                                            layout.output_device_button.width -
                                            2.0f * metrics.content_padding -
                                            metrics.chevron_icon_size;
    layout.output_device_button_chevron.y =
        layout.output_device_button.y +
        (layout.output_device_button.height - metrics.chevron_icon_size) *
            0.5f;
    ...
    layout.output_device_button_label.width =
        ...
        layout.output_device_button_label.x - metrics.content_padding;
    layout.output_device_button_label.height =
        layout.output_device_button.height;
  ...
  if (model != nullptr && model->expanded) {
    ...
    layout.app_volumes_panel.y = layout.app_volumes_title.y +
                                 layout.app_volumes_title.height +
                                 metrics.app_panel_gap;
    ...
    for (size_t index = 0; index < visible_sessions; ++index) {
      ...
      row->bounds.y =
          ...
          (float)index * metrics.app_volume_row_height;
      ...
      row->app_icon.y =
          row->bounds.y + (row->bounds.height - metrics.app_icon_size) * 0.5f;
      ...
      row->slider_full_range_line.x =
          row->app_volume_percent.x - metrics.app_row_percent_gap -
          metrics.app_row_slider_width;
      row->slider_full_range_line.y =
          row->bounds.y +
          (row->bounds.height - metrics.app_row_slider_line_height) * 0.5f;
      if (row->slider_full_range_line.x <
          row->app_icon.x + metrics.app_icon_size +
      ...
      row->app_label.width =
          row->slider_full_range_line.x - metrics.app_row_slider_gap -
          row->app_label.x;
      ...
      row->slider_thumb.y =
          row->bounds.y +
          (row->bounds.height - metrics.app_row_thumb_size) * 0.5f;
    ...
    layout.expand_button = reach_quick_settings_content_line(
        content_bounds, next_y + metrics.expand_button_gap,
  ...
  layout.expand_button_icon.x = layout.expand_button.x +
                                layout.expand_button.width -
                                2.0f * metrics.content_padding -
                                metrics.chevron_icon_size;
  layout.expand_button_icon.y =
      layout.expand_button.y +
      (layout.expand_button.height - metrics.chevron_icon_size) * 0.5f;

  layout.expand_button_label.x =
      layout.expand_button.x + metrics.text_padding;
  layout.expand_button_label.y = layout.expand_button.y;
  layout.expand_button_label.width =
      layout.expand_button_icon.x - layout.expand_button_label.x -
      metrics.content_padding;
