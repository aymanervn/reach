...
static reach_dock_order_key
reach_dock_order_key_for_item(const reach_dock_item_model *item) {
...
static void reach_dock_set_order_key(reach_dock_feature_model *model,
                                     size_t index,
...
size_t reach_dock_find_pinned_for_window(
    ...
    void *match_user) {
  if (pinned_apps == nullptr || window == nullptr ||
      window_matches_pinned == nullptr) {
...
static int32_t reach_dock_pinned_running(
    const reach_pinned_app_model *pinned_apps, size_t pinned_app_count,
    const reach_window_snapshot *open_windows, size_t open_window_count,
    size_t pinned_index,
  ...
  if (pinned_apps == nullptr || open_windows == nullptr ||
      pinned_index >= pinned_app_count) {
    return 0;
  }

  for (size_t index = 0; index < open_window_count; ++index) {
    if (reach_dock_find_pinned_for_window(
            pinned_apps, pinned_app_count, &open_windows[index],
...
static int32_t reach_dock_append_item(reach_dock_item_model *items,
                                      size_t *item_count,
                                      const reach_dock_item_model *item) {
...
static void reach_dock_append_pinned_items(
    reach_dock_item_model *items, size_t *item_count,
    const reach_pinned_app_model *pinned_apps, size_t pinned_app_count,
    const reach_window_snapshot *open_windows, size_t open_window_count,
    reach_dock_window_matches_pinned_fn window_matches_pinned,
    void *match_user) {
  ...
  for (size_t index = 0;
       index < pinned_app_count && *item_count < REACH_MAX_PINNED_APPS;
       ++index) {
    ...
    item.window =
        reach_dock_pinned_running(pinned_apps, pinned_app_count, open_windows,
                                  open_window_count, index,
                                  window_matches_pinned, match_user, &window_id)
            ? window_id
            : 0;
...
static void reach_dock_append_unpinned_open_windows(
  ...
  for (size_t index = 0;
       index < open_window_count && *item_count < REACH_MAX_PINNED_APPS;
       ++index) {
    if (reach_dock_find_pinned_for_window(
            pinned_apps, pinned_app_count, &open_windows[index],
            window_matches_pinned, match_user) != REACH_MAX_PINNED_APPS) {
      continue;
    }
...
static void reach_dock_build_candidate_items(
  ...
  reach_dock_append_pinned_items(
      items, item_count, pinned_apps, pinned_app_count, open_windows,
      open_window_count, window_matches_pinned, match_user);

  reach_dock_append_unpinned_open_windows(
      items, item_count, pinned_apps, pinned_app_count, open_windows,
      open_window_count, window_matches_pinned, match_user);
}

static void
reach_dock_apply_existing_order(reach_dock_feature_model *model,
  ...
  for (size_t order_index = 0; order_index < model->order_count &&
                               model->item_count < REACH_MAX_PINNED_APPS;
       ++order_index) {
...
void reach_dock_feature_model_build_items(
  ...
  reach_dock_build_candidate_items(
      candidates, &candidate_count, pinned_apps, pinned_app_count, open_windows,
      open_window_count, window_matches_pinned, match_user);
