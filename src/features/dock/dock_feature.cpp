...
/* A slot holds position: its animated width (icon box + trailing gap; the Now
   Playing widget's width + gap for slot 0) is the only geometry input — icons
   just sit on top. Dying slots linger (no backing item) until they land. */
struct reach_dock_slot
{
    int32_t lifecycle;
    reach_dock_order_key key; /* app slots; slot 0 (now playing) ignores it */
    float target_width;
};
...
struct reach_dock
{
    ...
    /* The slot pool (slot 0 = now playing) + its left-to-right order. Pool
       index is stable (it owns the width track); the order list is what the
       geometry accumulates over. slots_synced gates the first build: cold
       start / reset snaps, only later diffs animate. */
    ...
    /* Now-playing content reveal: armed when slot 0 starts growing; the delay
       (measured from the slot track, not hardcoded) fires the content scale
       at the reveal threshold so it lands together with the slot. */
                            ...
                            reach_feature_tick_result *out)
{
    ...
    /* Pre-tick activity: a track landing inside this tick must still produce
       one final frame (exact geometry, full reveal — the running indicator). */
    ...
    /* Settle: the release animation drained while nothing is pressed or sticky
     * — pin the opacity at zero and forget the feedback slot. */
    if (feedback_was_active &&
        !reach_animation_manager_active(manager, REACH_DOCK_ANIM_FEEDBACK_OPACITY) &&
    ...
    /* Snap finished: forget the dragged item's keys. */
    if (drag_snap_was_active &&
        !reach_animation_manager_active(manager, REACH_DOCK_ANIM_DRAG_SNAP))
    {
    ...
    /* Now-playing content reveal: measure the time until slot 0 crosses the
       reveal threshold, then scale the content in so it lands with the slot.
       Pacing-independent: a frame gap large enough to land the slot before the
       measured delay fires must still reveal the content. */
    {
        ...
        if (np_target > 0.0f && !content_active && content_value < 1.0f)
        {
            ...
            {
                /* The slot already landed (the reveal window fell inside one
                   large tick): ease the content in now. */
                animations->np_content_armed = 0;
...
static void reach_dock_capsule_reset(void *capsule)
{
    ...
    if (dock != nullptr)
    {
        ...
        /* Next build resyncs the slot pool snapped (no birth animations). */
        dock->slots_synced = 0;
    }
}
                                              ...
                                              reach_capsule_pointer_result *out)
{
    ...
    /* The pointer stream arrives in screen coordinates; everything below works
     * dock-local, so convert once against the layout captured at build time. */
...
const reach_feature_capsule_ops *reach_dock_capsule_ops(void)
{
    static const reach_feature_capsule_ops ops = {
        ...
        reach_dock_capsule_is_open, nullptr /* force_close: persistent */,
...
/*
 * Width animation + item-x reorder bookkeeping (moved verbatim out of
 * composition's host_dock_layout.cpp in the Phase-2 layout work).
 */

/* --- Slot pool: existence + width; the geometry below derives from it --- */
...
/* Items settle before they interact: while any slot animates, item hits are
   dead zones (the static cluster stays live). */
static void reach_dock_gate_animating_hit(reach_dock *dock, reach_dock_hit_result *hit)
{
...
/* Allocate a pool entry; when the pool is exhausted the oldest dying slot
   (first in order) snaps closed to make room. Never touches slot 0. */
...
/* Free landed dying slots + promote landed appearing slots (called from tick). */
static void reach_dock_settle_slots(reach_dock *dock)
{
    for (size_t pool = 0; pool < REACH_DOCK_SLOT_CAPACITY; ++pool)
    {
...
/*
 * Diff the slot pool against the freshly built item model. Kept slots stay,
 * born slots grow from 0, dead slots collapse in place (anchored after the
 * nearest preceding surviving slot), a key reappearing mid-collapse revives.
 * The first sync after create/reset snaps everything (no birth animation on
 * cold start); dpi/theme retargets snap steady slots and retarget moving ones.
 */
static void reach_dock_sync_slots(reach_dock *dock, float app_slot_width, float np_slot_width)
{
    ...
    /* Now-playing slot (0): retarget on visibility/width change, animated.
       Hiding drops the content instantly (dying content never fades); growth
       arms the content reveal, whose delay tick measures from the slot track
       rather than hardcoding a threshold time. */
    if (dock->slots[0].target_width != np_slot_width)
    {
        ...
        dock->np_content_armed = 0; /* re-measure the delay on the next tick */
    }

    /* Pass 1: mark deaths (live slots whose key left the model). */
    for (size_t pool = 1; pool < REACH_DOCK_SLOT_CAPACITY; ++pool)
    {
        reach_dock_slot *slot = &dock->slots[pool];
        if (slot->lifecycle != REACH_DOCK_SLOT_APPEARING &&
    ...
    /* Pass 2: anchor each dying slot to its nearest preceding survivor so it
       collapses in place across the reorder below. */
    ...
    /* Pass 3: rebuild the order from the model spine, reviving/growing as
       needed and splicing dying slots after their anchors. */
    ...
    for (size_t index = 0; index < item_count; ++index)
    {
        ...
        if (pool < REACH_DOCK_SLOT_CAPACITY)
        {
            ...
            {
                /* Revive: the key came back before the collapse landed. */
            ...
            if (slot->target_width != app_slot_width || slot->lifecycle != REACH_DOCK_SLOT_STEADY)
            {
                ...
                {
                    /* dpi/theme retarget on a resting slot: snap. */
                    reach_animation_manager_set(&dock->manager, reach_dock_slot_track(pool),
                                                app_slot_width);
                }
    ...
    /* Dying slots whose anchor itself died and left the order: append. */
    for (size_t pool = 1; pool < REACH_DOCK_SLOT_CAPACITY; ++pool)
    {
...
/* Snap every slot to its target (game mode / lifecycle resets). */
static void reach_dock_snap_slots(reach_dock *dock)
{
    reach_animation_manager_set(&dock->manager, REACH_DOCK_ANIM_NOW_PLAYING_CONTENT,
...
float reach_dock_item_reveal(reach_dock *dock, size_t item_index)
{
    ...
    if (pool >= REACH_DOCK_SLOT_CAPACITY)
    {
        return 1.0f; /* pool overflow: the item packed without a slot; draw it */
    }
                             ...
                             reach_dock_layout *layout)
{
    ...
    /* Slot sync: existence + widths (slot = icon box + trailing gap; slot 0 =
       now playing width + gap). All geometry below derives from the animated
       slot widths, left to right — positions and total width cannot disagree. */
    ...
    if (now_playing_render_width > 0.0f && np_width_now > 0.0f && np_content > 0.0f)
    {
        ...
        if (np_content < 1.0f)
        {
            /* Content scales in around its center once the slot crosses the
               reveal threshold. */
    ...
    /* Items without a slot (pool overflow): pack at the accumulated edge. */
...
/* Icon motion is an OFFSET from the item's (moving) slot position that decays
   to zero — reorders glide while the slot layer keeps shifting underneath. */
