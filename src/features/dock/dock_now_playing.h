...
typedef struct reach_dock_now_playing_render_context
{
    const reach_theme *theme;
    float dpi_scale;
    /* Visible content width from the now-playing slot (slot 0 value minus its
       trailing gap) — the widget clips to it while the slot animates. */
    float reveal_width;
...
/* Private pure helpers retained for focused feature logic tests. */
