    ...
    uint16_t *out_fallback_initial)
{
    ...
    if (item->pinned) {
        ...
        if (out_fallback_initial != nullptr) {
            *out_fallback_initial =
                pinned_index < REACH_MAX_PINNED_APPS
                    ? cache->pinned_icon_initials[pinned_index]
                    : '?';
        }

        return pinned_index < cache->pinned_icon_count
            ? cache->pinned_icons[pinned_index]
            : reach_icon_handle {};
    }
    ...
    if (out_fallback_initial != nullptr) {
        *out_fallback_initial =
            open_index < REACH_MAX_PINNED_APPS
                ? cache->open_window_initials[open_index]
                : '?';
    }

    return open_index < REACH_MAX_PINNED_APPS
        ? cache->open_window_icons[open_index]
        : reach_icon_handle {};
}
