...
static reach_rect_f32 reach_dock_local_rect(
    reach_rect_f32 rect,
...
static reach_rect_f32 reach_dock_center_square(reach_rect_f32 outer, float size)
{
    return reach_dock_rect(
        outer.x + (outer.width - size) * 0.5f,
        outer.y + (outer.height - size) * 0.5f,
        size,
        size);
}

static reach_rect_f32 reach_dock_union_rect(
    reach_rect_f32 a,
    reach_rect_f32 b)
{
...
static void reach_dock_push_rect(
...
static void reach_dock_push_vector_icon(
    reach_render_command_buffer *commands,
    reach_rect_f32 rect,
    uint32_t icon_id,
    reach_color color)
...
static void reach_dock_push_icon(
    reach_render_command_buffer *commands,
    reach_rect_f32 rect,
    uint64_t icon_id,
    reach_color color)
...
static void reach_dock_push_text(
    reach_render_command_buffer *commands,
    reach_rect_f32 rect,
    const uint16_t *text,
    float text_size,
    ...
    reach_color color)
{
    ...
    reach_copy_utf16(
        command.text,
        260,
        text != nullptr ? text : (const uint16_t *)L"");
    reach_render_command_buffer_push(commands, &command);
}

static void reach_dock_push_click_feedback(
    reach_render_command_buffer *commands,
    reach_rect_f32 rect,
    float radius,
    float opacity)
...
static void reach_dock_push_item_feedback(
    reach_render_command_buffer *commands,
    reach_rect_f32 rect,
    float radius,
...
static void reach_dock_push_running_indicator(
    ...
    reach_rect_f32 icon_box)
{
    ...
    int32_t focused =
        input->model->items[index].window == input->focused_window;
    float indicator_y =
        icon_box.y + icon_box.height + metrics.running_indicator_gap;
    float max_indicator_y = input->layout->bounds.height -
    ...
    color.b = 1.0f;
    color.a = focused
        ? metrics.running_indicator_focused_alpha
        : metrics.running_indicator_unfocused_alpha;

    reach_dock_push_rect(
        commands,
        reach_dock_rect(
            icon_box.x + (icon_box.width - metrics.running_indicator_size) *
                0.5f,
            indicator_y,
            metrics.running_indicator_size,
            metrics.running_indicator_size),
        color,
        metrics.running_indicator_size * 0.5f);
}
    ...
    float icon_box_radius)
{
    ...
    reach_rect_f32 icon_box = reach_dock_icon_box_for_slot(
    ...
    if (icon.id != 0) {
        ...
        fallback_background.a *=
            reach_dock_metrics_values.fallback_icon_background_alpha;
        reach_dock_push_rect(
            commands,
            icon_box,
            fallback_background,
...
static void reach_dock_push_background(
    ...
    float dock_radius)
{
    ...
    command.rect = reach_dock_rect(
        theme->dock_border_thickness * 0.5f,
        theme->dock_border_thickness * 0.5f,
...
static void reach_dock_push_system_buttons(
    const reach_dock_render_input *input,
    reach_render_command_buffer *commands,
    float icon_box_size,
    float icon_box_radius)
{
    const reach_theme *theme = input->theme;
    const reach_dock_layout *layout = input->layout;
    float system_icon_size =
        icon_box_size * reach_dock_metrics_values.system_icon_box_scale;

    reach_rect_f32 tray_box = reach_dock_icon_box_for_slot(
        layout->tray_button,
        layout->bounds,
        icon_box_size);
    reach_rect_f32 quick_settings_box = reach_dock_icon_box_for_slot(
    ...
    reach_dock_push_rect(
        commands,
        reach_dock_union_rect(tray_box, quick_settings_box),
        theme->tray_button_background,
        icon_box_radius);
    reach_dock_push_vector_icon(
        commands,
        reach_dock_center_square(tray_box, system_icon_size),
    ...
    if (input->click_feedback_index == input->tray_feedback_index) {
        reach_dock_push_click_feedback(
            commands,
            tray_box,
            icon_box_radius,
            input->click_feedback_opacity);
...
static void reach_dock_push_clock(
    const reach_dock_render_input *input,
    reach_render_command_buffer *commands)
{
    const reach_dock_metrics &metrics = reach_dock_metrics_values;
    reach_rect_f32 clock = reach_dock_local_rect(
        input->layout->clock,
        input->layout->bounds);
    ...
    reach_dock_push_text(
        ...
        reach_dock_rect(
            clock.x,
            clock.y + metrics.clock_time_top_offset,
            clock.width,
            clock.height * metrics.clock_time_height_ratio),
        input->time_text,
        metrics.clock_time_text_size,
    ...
    reach_dock_push_text(
        ...
        reach_dock_rect(
            clock.x,
            clock.y + clock.height * metrics.clock_date_top_ratio,
            clock.width,
            clock.height * metrics.clock_date_height_ratio),
        input->date_text,
        metrics.clock_date_text_size,
        metrics.clock_date_text_weight,
        0,
        input->theme->dock_clock_date);
}

static void reach_dock_push_power_button(
    const reach_dock_render_input *input,
    reach_render_command_buffer *commands,
    float icon_box_size)
{
    ...
    float system_icon_size =
        icon_box_size * reach_dock_metrics_values.system_icon_box_scale;
    reach_rect_f32 power_box = reach_dock_icon_box_for_slot(
        layout->power_button,
        layout->bounds,
        icon_box_size);
    ...
    reach_dock_push_rect(
        commands,
        power_box,
        theme->dock_power_button_background,
        theme->dock_power_button_corner_radius);
    reach_dock_push_vector_icon(
        commands,
        reach_dock_center_square(power_box, system_icon_size),
        REACH_VECTOR_ICON_POWER,
        theme->dock_power_glyph);
...
reach_result reach_dock_build_render_commands(
    const reach_dock_render_input *input,
    reach_render_command_buffer *out_commands)
{
    ...
    float dock_radius =
        reach_theme_dock_corner_radius(theme, layout->bounds.height);
    float icon_box_size =
        reach_theme_icon_box_size(theme, layout->bounds.height);
    float icon_box_radius =
        reach_theme_icon_box_corner_radius(theme, icon_box_size);
    ...
    reach_dock_push_system_buttons(
        input,
        out_commands,
        icon_box_size,
        icon_box_radius);
    ...
    reach_rect_f32 separator =
        reach_dock_local_rect(layout->system_separator, layout->bounds);
    reach_dock_push_rect(
        out_commands,
        separator,
        theme->dock_system_separator,
