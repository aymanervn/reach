...
static float reach_clipboard_render_scale(float value, float dpi_scale)
{
                                                   ...
                                                   reach_render_command_buffer *commands)
{
    ...
    command.radius = reach_clipboard_render_scale(theme->clipboard_panel_radius, input->dpi_scale);
    ...
    command.text_size =
        reach_clipboard_render_scale(theme->clipboard_title_text_size, input->dpi_scale);
    ...
    for (size_t index = 0; index < model->count; ++index)
    {
        ...
        command.radius =
            reach_clipboard_render_scale(theme->clipboard_item_radius, input->dpi_scale);
        reach_render_command_buffer_push(commands, &command);
        ...
        {
            float close_size = metrics.close_button_size;
            float close_margin = metrics.close_button_margin;
            reach_rect_f32 close_rect = {local_item.x + local_item.width - close_size -
                                             close_margin,
                                         local_item.y + close_margin, close_size, close_size};
            float close_hover_alpha = metrics.close_button_hover_alpha;
            command = {};
            command.type = REACH_RENDER_COMMAND_RECT;
            command.rect = close_rect;
            command.color.a = close_hover_alpha * hover;
            command.radius = reach_clipboard_render_scale(4.0f, input->dpi_scale);
            reach_render_command_buffer_push(commands, &command);

            float inset = close_size * metrics.close_button_inset_ratio;
            command = {};
            command.type = REACH_RENDER_COMMAND_VECTOR_ICON;
            command.rect = {close_rect.x + inset, close_rect.y + inset,
                            close_rect.width - inset * 2.0f, close_rect.height - inset * 2.0f};
        ...
        float padding = metrics.thumbnail_padding;
        if (item_data->kind == REACH_CLIPBOARD_ITEM_IMAGE && item_data->thumbnail_id != 0)
        {
            ...
            command = {};
            command.type = REACH_RENDER_COMMAND_TEXT;
            command.rect = {local_item.x + padding + thumbnail + metrics.thumbnail_padding,
            ...
            command.text_size =
                reach_clipboard_render_scale(theme->clipboard_body_text_size, input->dpi_scale);
            command.text_alignment = input->text_alignment_leading;
        ...
        else
        {
            ...
            command.rect = {local_item.x + padding, local_item.y + padding,
                            local_item.width - padding * 2.0f, local_item.height - padding * 2.0f};
            command.color = theme->clipboard_primary_text;
            command.text_size =
                reach_clipboard_render_scale(theme->clipboard_body_text_size, input->dpi_scale);
            command.text_alignment = input->text_alignment_leading;
