...
/*
 * The launcher model + its ops, relocated from the core reach_ui_state aggregate
 * (E phase 3). Same logic as the former reach_ui_state_*launcher* functions, now
 * operating on the capsule-owned reach_launcher_state. reach_launcher_model stays
 * a core data type (so core layout can name it); only ownership + the ops moved.
 */
...
/*
 * Capsule-level ops: the public API targets the capsule; the *_state versions
 * above are feature-internal (launcher_common.h).
 */
                                           ...
                                           reach_search_candidate *out_results, size_t *out_count)
{
    ...
    /* Stale (a newer query superseded it) or the launcher closed meanwhile. */
...
/*
 * Search-box caret + focus + interaction resets (moved out of composition in
 * the class-driven interface phase).
 */
...
/*
 * Typed-character / editing-key input into the launcher's text-edit model.
 * Submit/cancel/navigate are NOT routed here: ENTER/ESCAPE/ARROW arrive as
 * their own ui events and flow through reach_launcher_handle_event. A text
 * change updates the query and runs the attached search; an empty query
 * cancels the search and clears the results.
 */
                                      ...
                                      reach_launcher_text_event_result *out_result)
{
    ...
    /* Any accepted edit shows the caret immediately and redraws the box. */
    reach_launcher_reset_caret(launcher);
    out_result->redraw = 1;
...
const reach_feature_capsule_ops *reach_launcher_capsule_ops(void)
{
    static const reach_feature_capsule_ops ops = {
        ...
        nullptr /* force_close: launcher close runs composition focus policy */,
