...
enum
{
    REACH_POPUP_TRANSITION_Y = 0,
    REACH_POPUP_TRANSITION_OPACITY = 1
};

static const float REACH_POPUP_TRANSITION_OFFSET = 8.0f;

static float reach_popup_transition_origin(const reach_popup_transition *transition)
{
    return transition != nullptr && transition->direction == REACH_POPUP_DROP_DOWN
               ? -REACH_POPUP_TRANSITION_OFFSET
               : REACH_POPUP_TRANSITION_OFFSET;
}
...
void reach_popup_transition_reset(reach_popup_transition *transition)
{
    ...
    reach_animation_manager_set(&transition->animations, REACH_POPUP_TRANSITION_Y,
                                reach_popup_transition_origin(transition));
    reach_animation_manager_set(&transition->animations, REACH_POPUP_TRANSITION_OPACITY, 0.0f);
}
...
int32_t reach_popup_transition_set_open(reach_popup_transition *transition, int32_t open)
{
    ...
    if (target_open)
    {
        if (!transition->visible)
        {
            ...
            reach_animation_manager_set(&transition->animations, REACH_POPUP_TRANSITION_Y,
                                        reach_popup_transition_origin(transition));
            reach_animation_manager_set(&transition->animations, REACH_POPUP_TRANSITION_OPACITY,
                                        0.0f);
        }
        reach_animation_manager_animate_to(&transition->animations, REACH_POPUP_TRANSITION_Y, 0.0f,
                                           transition->open_seconds, REACH_EASING_EASE_OUT);
        reach_animation_manager_animate_to(&transition->animations,
                                           REACH_POPUP_TRANSITION_OPACITY, 1.0f,
                                           transition->open_seconds, REACH_EASING_EASE_OUT);
    }
    else if (transition->visible)
    {
        reach_animation_manager_animate_to(&transition->animations, REACH_POPUP_TRANSITION_Y,
                                           reach_popup_transition_origin(transition),
                                           transition->close_seconds, REACH_EASING_EASE_IN);
        reach_animation_manager_animate_to(&transition->animations,
                                           REACH_POPUP_TRANSITION_OPACITY, 0.0f,
                                           transition->close_seconds, REACH_EASING_EASE_IN);
    }
    return 1;
}
...
int32_t reach_popup_transition_tick(reach_popup_transition *transition, double delta_seconds)
{
    ...
    if (!transition->target_open && transition->visible && !active &&
        reach_animation_manager_value(&transition->animations, REACH_POPUP_TRANSITION_OPACITY) <=
            0.001f)
    {
        transition->visible = 0;
    }
                                         ...
                                         reach_feature_surface_geometry *geometry)
{
    ...
    geometry->presentation.y_offset =
        reach_animation_manager_value(&transition->animations, REACH_POPUP_TRANSITION_Y) *
