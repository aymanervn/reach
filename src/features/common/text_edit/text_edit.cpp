...
/* Removes [start,end) from the buffer and places the caret at start. */
...
reach_text_edit_event reach_text_edit_insert_text(reach_text_edit *edit, const uint16_t *text)
{
    ...
    while (text[index] != 0 && edit->length < max)
    {
        ...
        /* Single-line: ignore control/line characters. */
...
/* Moves the caret, extending or collapsing the selection per the shift modifier. */
                                                 ...
                                                 reach_text_edit_modifiers modifiers)
{
    ...
    case REACH_TEXT_EDIT_KEY_LEFT:
        /* Collapse selection to its left edge when not extending. */
