...
/*
 * Placement: bounds + item slots from the open context. The popup body stacks
 * item_count rows over a notch; anchored menus sit 8px (scaled) above the dock
 * top, aligned to anchor_x by anchor_ratio and clamped into the monitor;
 * unanchored item menus hang off the pointer without clamping (legacy path).
 */
