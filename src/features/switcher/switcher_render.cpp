...
reach_result reach_switcher_build_render_commands(const reach_switcher_render_input *input, reach_render_command_buffer *out_commands)
{
    if (input == nullptr || input->theme == nullptr || input->model == nullptr || input->items == nullptr || out_commands == nullptr) {
    ...
    if (visible_count > 0) {
        ...
        for (size_t visible_index = 0; visible_index < visible_count; ++visible_index) {
            ...
            if (selected) {
                ...
                reach_copy_utf16(command.text, 260, input->items[index].label[0] != 0 ? input->items[index].label : (const uint16_t *)L"App");
                reach_render_command_buffer_push(out_commands, &command);
            }
        }
    }
