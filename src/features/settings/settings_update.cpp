...
{
    if (model == nullptr || reach_settings_model_update_busy(model)) return;
...
void reach_settings_model_begin_update_install(reach_settings_model *model)
{
    if (model == nullptr || reach_settings_model_update_busy(model) ||
        reach_settings_model_selected_update_count(model) == 0) return;
...
{
    if (left->revision_number != right->revision_number) return 0;
    for (size_t index = 0; index < REACH_WINDOWS_UPDATE_ID_CAPACITY; ++index)
    {
        if (left->update_id[index] != right->update_id[index]) return 0;
        if (left->update_id[index] == 0) return 1;
    }
    return 1;
}
...
{
    if (model == nullptr || result == nullptr) return;
...
{
    if (model == nullptr || reach_settings_model_update_busy(model) ||
        index >= model->update_list.count) return;
    ...
    if (update->state != REACH_WINDOWS_UPDATE_DISCOVERED &&
        ...
        update->state != REACH_WINDOWS_UPDATE_FAILED) return;
    ...
    for (size_t index = 0; index < model->update_list.count; ++index)
        if (model->update_list.updates[index].selected &&
            (model->update_list.updates[index].state == REACH_WINDOWS_UPDATE_SELECTED ||
             model->update_list.updates[index].state == REACH_WINDOWS_UPDATE_FAILED)) ++count;
    return count;
}
...
{
    if (model == nullptr || visible_count == 0 || model->update_list.count <= visible_count) return;
    size_t maximum = model->update_list.count - visible_count;
    if (delta < 0)
    {
        size_t amount = (size_t)(-delta);
        model->update_scroll_offset = amount > model->update_scroll_offset
                                          ? 0
                                          : model->update_scroll_offset - amount;
    }
    else
    {
