...
reach_result reach_app_create(const reach_shell_desc *desc,
                              reach_app **out_app) {
  ...
  *out_app = nullptr;
  ...
  reach_result result =
      reach_windows_create_tray_provider(&dependencies.tray_provider);
  ...
  if (result == REACH_OK) {
    result =
        reach_windows_create_wallpaper_surface(&dependencies.wallpaper_surface);
  }

  if (result == REACH_OK) {
    result = reach_windows_create_platform_window(
        REACH_SURFACE_LAUNCHER, &dependencies.launcher_window);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_dcomp_render_backend(
        dependencies.launcher_window.window, &dependencies.launcher_renderer);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_platform_window(REACH_SURFACE_DOCK,
                                                  &dependencies.dock_window);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_dcomp_render_backend(
        dependencies.dock_window.window, &dependencies.dock_renderer);
  }
  if (result == REACH_OK) {
    result =
        reach_windows_create_dock_reveal_edge(&dependencies.dock_reveal_edge);
  }
  if (result == REACH_OK) {
  ...
  if (result == REACH_OK) {
    result = reach_windows_create_dcomp_render_backend(
        dependencies.tray_window.window, &dependencies.tray_renderer);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_platform_window(
        REACH_SURFACE_SWITCHER, &dependencies.switcher_window);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_dcomp_render_backend(
        dependencies.switcher_window.window, &dependencies.switcher_renderer);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_platform_window(
        REACH_SURFACE_CONTEXT_MENU, &dependencies.context_menu_window);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_dcomp_render_backend(
        dependencies.context_menu_window.window,
        &dependencies.context_menu_renderer);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_platform_window(
        REACH_SURFACE_QUICK_SETTINGS, &dependencies.quick_settings_window);
  }
  if (result == REACH_OK) {
    result = reach_windows_create_dcomp_render_backend(
  ...
  if (result == REACH_OK) {
    result =
        reach_windows_create_search_provider(&dependencies.search_provider);
  }
  if (result == REACH_OK) {
  ...
  if (result == REACH_OK) {
    ...
    const uint16_t *config_path =
        desc != nullptr && desc->config_path != nullptr ? desc->config_path
                                                        : default_path;
    if (desc == nullptr || desc->config_path == nullptr) {
      result = reach_windows_default_config_path(default_path, 260);
    }
    if (result == REACH_OK) {
      result = reach_windows_create_config_store(config_path,
  ...
  if (result == REACH_OK) {
    result =
        reach_windows_create_explorer_service(&dependencies.explorer_service);
  }
  if (result == REACH_OK) {
    result =
        reach_windows_create_wallpaper_service(&dependencies.wallpaper_service);
  }
  if (result == REACH_OK) {
  ...
  if (result == REACH_OK) {
    result =
        reach_windows_create_system_controls(&dependencies.system_controls);
  }
  if (result == REACH_OK) {
    result =
        reach_shell_create_with_dependencies(desc, &dependencies, &app->shell);
    if (result == REACH_OK) {
  ...
  if (result != REACH_OK) {
    if (dependencies.launcher_window.ops.destroy != nullptr) {
      dependencies.launcher_window.ops.destroy(
          dependencies.launcher_window.window);
    }
    if (dependencies.launcher_renderer.ops.destroy != nullptr) {
      dependencies.launcher_renderer.ops.destroy(
          dependencies.launcher_renderer.backend);
    }
    ...
    if (dependencies.dock_renderer.ops.destroy != nullptr) {
      dependencies.dock_renderer.ops.destroy(
          dependencies.dock_renderer.backend);
    }
    ...
    if (dependencies.tray_renderer.ops.destroy != nullptr) {
      dependencies.tray_renderer.ops.destroy(
          dependencies.tray_renderer.backend);
    }
    if (dependencies.switcher_window.ops.destroy != nullptr) {
      dependencies.switcher_window.ops.destroy(
          dependencies.switcher_window.window);
    }
    if (dependencies.switcher_renderer.ops.destroy != nullptr) {
      dependencies.switcher_renderer.ops.destroy(
          dependencies.switcher_renderer.backend);
    }
    if (dependencies.context_menu_window.ops.destroy != nullptr) {
      dependencies.context_menu_window.ops.destroy(
          dependencies.context_menu_window.window);
    }
    ...
    if (dependencies.window_manager.ops.destroy != nullptr) {
      dependencies.window_manager.ops.destroy(
          dependencies.window_manager.manager);
    }
    ...
    if (dependencies.tray_provider.ops.destroy != nullptr) {
      dependencies.tray_provider.ops.destroy(
          dependencies.tray_provider.provider);
    }
    if (dependencies.search_provider.ops.destroy != nullptr) {
      dependencies.search_provider.ops.destroy(
          dependencies.search_provider.provider);
    }
    ...
    if (dependencies.icon_provider.ops.destroy != nullptr) {
      dependencies.icon_provider.ops.destroy(
          dependencies.icon_provider.provider);
    }
    if (dependencies.explorer_service.ops.destroy != nullptr) {
      dependencies.explorer_service.ops.destroy(
          dependencies.explorer_service.service);
    }
    if (dependencies.wallpaper_service.ops.destroy != nullptr) {
      dependencies.wallpaper_service.ops.destroy(
          dependencies.wallpaper_service.service);
    }
    if (dependencies.wallpaper_surface.ops.destroy != nullptr) {
      dependencies.wallpaper_surface.ops.destroy(
          dependencies.wallpaper_surface.surface);
    }
    ...
    if (dependencies.power_session.ops.destroy != nullptr) {
      dependencies.power_session.ops.destroy(
          dependencies.power_session.session);
    }
    ...
    if (dependencies.system_controls.destroy != nullptr) {
      dependencies.system_controls.destroy(
          dependencies.system_controls.userdata);
    }
...
reach_result reach_app_start(reach_app *app) {
  ...
  if (result == REACH_OK && app->input_source.ops.start != nullptr) {
    result = app->input_source.ops.start(app->input_source.source,
                                         reach_app_on_input, app);
  }
  return result;
}
...
int32_t reach_app_has_pending_events(const reach_app *app) {
  return app != nullptr && app->shell != nullptr &&
         reach_shell_has_pending_events(app->shell);
}

int32_t reach_app_needs_frame(const reach_app *app) {
  return app != nullptr && app->shell != nullptr &&
         reach_shell_needs_frame(app->shell);
}
