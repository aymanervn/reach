...
static LRESULT CALLBACK reach_window_proc(HWND hwnd, UINT message,
                                          WPARAM wparam, LPARAM lparam) {
  reach_platform_window *window = reinterpret_cast<reach_platform_window *>(
  ...
  case WM_KEYDOWN:
    if (window != nullptr) {
      reach_ui_event event = {};
      event.modifiers = (GetKeyState(VK_CONTROL) & 0x8000) != 0
  ...
  case WM_MEASUREITEM: {
    ...
    if (measure != nullptr && measure->CtlType == ODT_MENU) {
      const wchar_t *text =
          reinterpret_cast<const wchar_t *>(measure->itemData);
      SIZE size = {};
      HDC dc = GetDC(hwnd);
...
static reach_result
reach_platform_window_set_bounds(reach_platform_window *window,
  ...
  BOOL ok = SetWindowPos(window->hwnd, HWND_TOPMOST, (int)bounds.x,
                         (int)bounds.y, width, height, SWP_NOACTIVATE);
  window->width = width;
...
static reach_result
reach_platform_window_apply_rounded_corners(reach_platform_window *window,
                                            float radius) {
  if (window == nullptr || window->hwnd == nullptr) {
    return REACH_INVALID_ARGUMENT;
  }
  ...
  */
  ...
  (void)DwmSetWindowAttribute(window->hwnd, DWMWA_WINDOW_CORNER_PREFERENCE,
  ...
  HRGN region = CreateRoundRectRgn(0, 0, window->width + 1, window->height + 1,
                                   diameter, diameter);
...
static reach_result
reach_platform_window_set_opacity(reach_platform_window *window,
                                  float opacity) {
  if (window == nullptr || window->hwnd == nullptr) {
    return REACH_INVALID_ARGUMENT;
  }
  ...
  return SetLayeredWindowAttributes(window->hwnd, 0, alpha, LWA_ALPHA)
             ? REACH_OK
             : REACH_ERROR;
}

static reach_result
reach_platform_window_set_blur_enabled(reach_platform_window *window,
                                       int32_t enabled) {
  if (window == nullptr || window->hwnd == nullptr) {
    return REACH_INVALID_ARGUMENT;
  }
  ...
  return SUCCEEDED(hr) || hr == DWM_E_COMPOSITIONDISABLED ? REACH_OK
                                                          : REACH_ERROR;
}
...
static int32_t
reach_platform_window_has_pending_events(const reach_platform_window *window) {
  return window != nullptr && window->pending_event_count > 0;
}

static reach_result
reach_platform_window_dispatch_events(reach_platform_window *window) {
  if (window == nullptr) {
    return REACH_INVALID_ARGUMENT;
  }
...
static reach_result
reach_platform_window_set_pointer_move_enabled(reach_platform_window *window,
                                               int32_t enabled) {
  if (window == nullptr) {
    return REACH_INVALID_ARGUMENT;
  }
...
void *
reach_windows_platform_window_native_handle(reach_platform_window *window) {
...
static int32_t reach_platform_window_queue_event(reach_platform_window *window,
  ...
  if (window == nullptr || event == nullptr ||
      event->type == REACH_UI_EVENT_NONE) {
    return 0;
  }

  if (event->type == REACH_UI_EVENT_POINTER_MOVE &&
      window->pending_event_count > 0 &&
      window->pending_events[window->pending_event_count - 1].type ==
          REACH_UI_EVENT_POINTER_MOVE) {
  ...
  if (window->pending_event_count < REACH_PLATFORM_WINDOW_MAX_PENDING_EVENTS) {
    ...
    window->pending_events[REACH_PLATFORM_WINDOW_MAX_PENDING_EVENTS - 1] =
        *event;
  }
  return 1;
}
...
static reach_result
reach_platform_window_post_event(reach_platform_window *window,
...
reach_result
reach_windows_create_platform_window(reach_surface_role role,
  ...
  *out_port = {};
  ...
  window->hwnd =
      CreateWindowExW(reach_window_ex_style(role), reach_window_class_name(),
                      L"Reach", WS_POPUP, 0, 0, 1, 1, nullptr, nullptr,
                      GetModuleHandleW(nullptr), nullptr);
  if (window->hwnd == nullptr) {
    delete window;
    return REACH_ERROR;
  }
  SetWindowLongPtrW(window->hwnd, GWLP_USERDATA,
  ...
  out_port->ops.apply_rounded_corners =
      reach_platform_window_apply_rounded_corners;
  ...
  out_port->ops.set_pointer_move_enabled =
      reach_platform_window_set_pointer_move_enabled;
