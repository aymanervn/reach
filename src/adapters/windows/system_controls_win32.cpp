...
struct reach_com_scope
{
    ...
    reach_com_scope() : uninitialize(0), result(REACH_OK)
    {
...
static void reach_system_controls_notify(reach_system_controls_adapter *adapter, uint32_t flags)
{
...
static int32_t reach_bluetooth_worker_find_radio(winrt::Windows::Devices::Radios::Radio *out_radio)
{
...
static reach_result reach_bluetooth_worker_get_state(winrt::Windows::Devices::Radios::Radio *radio,
                                                     reach_bluetooth_state *out_state)
{
    ...
    *out_state = {};
    ...
    try
    {
        ...
        out_state->enabled =
            radio->State() == winrt::Windows::Devices::Radios::RadioState::On ? 1 : 0;
    }
    catch (winrt::hresult_error const &)
    {
        *radio = nullptr;
...
static reach_result
reach_bluetooth_worker_set_enabled(winrt::Windows::Devices::Radios::Radio *radio, int32_t enabled)
{
    namespace radios = winrt::Windows::Devices::Radios;
...
static DWORD WINAPI reach_bluetooth_worker_thread(void *context);

static void reach_system_controls_stop_bluetooth_worker(reach_system_controls_adapter *adapter);

static reach_result
reach_system_controls_start_bluetooth_worker(reach_system_controls_adapter *adapter)
{
    if (adapter == nullptr)
    {
        return REACH_INVALID_ARGUMENT;
    ...
    if (adapter->bluetooth_stop == nullptr || adapter->bluetooth_request == nullptr ||
    ...
    adapter->bluetooth_thread =
        CreateThread(nullptr, 0, reach_bluetooth_worker_thread, adapter, 0, nullptr);
    if (adapter->bluetooth_thread == nullptr)
    {
        reach_system_controls_stop_bluetooth_worker(adapter);
        return REACH_ERROR;
...
static void WINAPI reach_system_controls_wlan_notification(PWLAN_NOTIFICATION_DATA data,
                                                           PVOID context)
{
    (void)data;
    reach_system_controls_notify(static_cast<reach_system_controls_adapter *>(context),
...
static DWORD WINAPI reach_system_controls_addr_change_thread(void *context)
{
    ...
    if (adapter == nullptr || adapter->addr_change == nullptr ||
        adapter->addr_change_stop == nullptr)
    {
        return 0;
    }

    HANDLE waits[2] = {adapter->addr_change_stop, adapter->addr_change};
    ...
    while (WaitForSingleObject(adapter->addr_change_stop, 0) == WAIT_TIMEOUT)
    {
        ...
        DWORD result = NotifyAddrChange(&notification_handle, &adapter->addr_change_overlapped);
        ...
        {
            reach_system_controls_notify(adapter, REACH_SYSTEM_CONTROLS_CHANGE_NETWORK);
        }
        else
        {
            CancelIPChangeNotify(&adapter->addr_change_overlapped);
...
static DWORD WINAPI reach_bluetooth_worker_thread(void *context)
{
    ...
    if (reach_bluetooth_worker_find_radio(&radio))
    {
        try
        {
            state_token = radio.StateChanged(
                [adapter](winrt::Windows::Devices::Radios::Radio const &,
    ...
    HANDLE waits[2] = {adapter->bluetooth_stop, adapter->bluetooth_request};
                                                   ...
                                                   const DOT11_SSID *ssid)
{
    ...
    *state = {};
    state->kind = REACH_NETWORK_KIND_WIFI;
    state->connected = 1;
...
static reach_network_kind reach_system_controls_get_adapter_network_kind(void)
{
    ULONG buffer_size = 0;
    ULONG flags = GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER;

    ULONG result = GetAdaptersAddresses(AF_UNSPEC, flags, nullptr, nullptr, &buffer_size);
    ...
    reach_network_kind kind = REACH_NETWORK_KIND_NONE;
    if (result == NO_ERROR)
    {
        ...
        reach_windows_network_adapter *adapters =
            new (std::nothrow) reach_windows_network_adapter[adapter_count];
        if (adapters != nullptr)
        {
            size_t index = 0;
            ...
            {
                adapters[index].if_type = adapter->IfType;
                adapters[index].oper_up = adapter->OperStatus == IfOperStatusUp;
                adapters[index].has_unicast = adapter->FirstUnicastAddress != nullptr;
                ++index;
            }
            kind = reach_windows_network_kind_from_adapters(adapters, adapter_count);
            delete[] adapters;
        }
    }
...
static int32_t reach_system_controls_get_wlan_network_state(reach_network_state *out_state)
{
    ...
    DWORD wlan_result = WlanOpenHandle(2, nullptr, &negotiated_version, &wlan);
    ...
    if (wlan_result == ERROR_SUCCESS && interfaces != nullptr)
    {
        for (DWORD index = 0; index < interfaces->dwNumberOfItems; ++index)
        {
            ...
            DWORD query_result = WlanQueryInterface(
                wlan, &info->InterfaceGuid, wlan_intf_opcode_current_connection, nullptr,
                &data_size, reinterpret_cast<PVOID *>(&attributes), &opcode_type);
...
static reach_result reach_system_controls_get_network_state(void *userdata,
                                                            reach_network_state *out_state)
{
    ...
    reach_system_controls_set_disconnected_network(out_state);

    if (reach_system_controls_get_wlan_network_state(out_state))
    {
        return REACH_OK;
    }

    reach_network_kind adapter_kind = reach_system_controls_get_adapter_network_kind();
    if (adapter_kind == REACH_NETWORK_KIND_WIFI)
    {
        reach_system_controls_set_wifi_network(out_state, 100, nullptr);
    }
    else if (adapter_kind == REACH_NETWORK_KIND_ETHERNET)
    {
        reach_system_controls_set_ethernet_network(out_state);
    }

    return REACH_OK;
}

static reach_result reach_system_controls_get_bluetooth_state(void *userdata,
                                                              reach_bluetooth_state *out_state)
{
    if (out_state == nullptr)
    {
    ...
    *out_state = {};
    reach_system_controls_adapter *adapter = static_cast<reach_system_controls_adapter *>(userdata);
    if (adapter == nullptr || adapter->bluetooth_thread == nullptr ||
        adapter->bluetooth_request == nullptr || adapter->bluetooth_complete == nullptr ||
        adapter->bluetooth_stop == nullptr || !adapter->bluetooth_lock_initialized)
    {
        return REACH_OK;
    }
    ...
    HANDLE waits[3] = {adapter->bluetooth_complete, adapter->bluetooth_stop,
                       adapter->bluetooth_thread};
...
static reach_result reach_system_controls_set_bluetooth_enabled(void *userdata, int32_t enabled)
{
    reach_system_controls_adapter *adapter = static_cast<reach_system_controls_adapter *>(userdata);
    if (adapter == nullptr || adapter->bluetooth_thread == nullptr ||
        adapter->bluetooth_request == nullptr || adapter->bluetooth_complete == nullptr ||
...
static reach_result reach_system_controls_request_bluetooth_enabled(void *userdata, int32_t enabled)
{
    reach_system_controls_adapter *adapter = static_cast<reach_system_controls_adapter *>(userdata);
    if (adapter == nullptr || adapter->bluetooth_thread == nullptr ||
        adapter->bluetooth_request == nullptr || adapter->bluetooth_stop == nullptr ||
                                                          ...
                                                          reach_power_state *out_state)
{
    ...
    *out_state = {};
    ...
    out_state->has_battery = (status.BatteryFlag & 128) == 0 && status.BatteryFlag != 255 ? 1 : 0;
...
static int32_t reach_system_controls_variant_i32(const VARIANT *variant, int32_t *out_value)
{
...
static HRESULT reach_system_controls_connect_wmi(IWbemServices **out_services)
{
    ...
    *out_services = nullptr;
    ...
    hr = locator->ConnectServer(namespace_name, nullptr, nullptr, nullptr, 0, nullptr, nullptr,
                                &services);
...
static HRESULT reach_system_controls_find_brightness_instance(IWbemServices *services,
                                                              int32_t *out_percent,
                                                              BSTR *out_instance_name)
{
    ...
    BSTR query =
        SysAllocString(L"SELECT Active, CurrentBrightness, InstanceName FROM WmiMonitorBrightness");
    ...
    while (true)
    {
        ...
        HRESULT brightness_hr =
            object->Get(L"CurrentBrightness", 0, &brightness_variant, nullptr, nullptr);
            ...
            reach_system_controls_variant_i32(&brightness_variant, &percent))
        {
            ...
            if (!found || active)
            {
                ...
                if (SUCCEEDED(instance_hr) && instance_variant.vt == VT_BSTR &&
...
static reach_result reach_system_controls_get_brightness_state(void *userdata,
                                                               reach_brightness_state *out_state)
{
    ...
    *out_state = {};
    ...
    int32_t percent = 0;
    hr = reach_system_controls_find_brightness_instance(services.Get(), &percent, nullptr);
    if (FAILED(hr))
    {
...
static reach_result reach_system_controls_set_brightness_level(void *userdata, float level)
{
    (void)userdata;
    ...
    hr = reach_system_controls_find_brightness_instance(services.Get(), nullptr, &instance_name);
    ...
    BSTR language = SysAllocString(L"WQL");
    BSTR query = SysAllocString(L"SELECT __PATH, InstanceName FROM WmiMonitorBrightnessMethods");
    if (language == nullptr || query == nullptr)
    {
        SysFreeString(instance_name);
        if (language != nullptr)
    ...
    ComPtr<IEnumWbemClassObject> enumerator;
    hr = services->ExecQuery(language, query, WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY,
                             nullptr, &enumerator);
    SysFreeString(language);
    SysFreeString(query);
    if (FAILED(hr))
    ...
    while (true)
    {
        ...
        HRESULT instance_hr = object->Get(L"InstanceName", 0, &instance_variant, nullptr, nullptr);
        if (SUCCEEDED(path_hr) && SUCCEEDED(instance_hr) && path_variant.vt == VT_BSTR &&
            instance_variant.vt == VT_BSTR && path_variant.bstrVal != nullptr &&
            instance_variant.bstrVal != nullptr &&
    ...
    if (SUCCEEDED(hr))
    {
        hr = services->ExecMethod(method_path, method_name, 0, nullptr, in_parameters.Get(),
                                  nullptr, nullptr);
    }
...
static INPUT reach_system_controls_key_input(WORD virtual_key, DWORD flags)
{
    INPUT input = {};
...
static reach_result reach_system_controls_open_project_menu(void *userdata)
{
    (void)userdata;

    INPUT inputs[] = {
        reach_system_controls_key_input(VK_LWIN, 0),
        reach_system_controls_key_input(0x50, 0),
        reach_system_controls_key_input(0x50, KEYEVENTF_KEYUP),
        reach_system_controls_key_input(VK_LWIN, KEYEVENTF_KEYUP),
    };

    UINT sent = SendInput((UINT)(sizeof(inputs) / sizeof(inputs[0])), inputs, sizeof(INPUT));
    return sent == (sizeof(inputs) / sizeof(inputs[0])) ? REACH_OK : REACH_ERROR;
}

static reach_result reach_system_controls_open_system_quick_settings(void *userdata)
{
    (void)userdata;

    INPUT inputs[] = {
        reach_system_controls_key_input(VK_LWIN, 0),
        reach_system_controls_key_input(0x41, 0),
        reach_system_controls_key_input(0x41, KEYEVENTF_KEYUP),
        reach_system_controls_key_input(VK_LWIN, KEYEVENTF_KEYUP),
    };
...
static reach_result
reach_system_controls_start_watching(void *userdata, reach_system_controls_change_callback callback,
                                     void *callback_user)
{
    reach_system_controls_adapter *adapter = static_cast<reach_system_controls_adapter *>(userdata);
    ...
    DWORD negotiated_version = 0;
    DWORD wlan_result = WlanOpenHandle(2, nullptr, &negotiated_version, &adapter->wlan);
    if (wlan_result == ERROR_SUCCESS && adapter->wlan != nullptr)
    {
        DWORD previous_sources = 0;
        wlan_result = WlanRegisterNotification(
            adapter->wlan, WLAN_NOTIFICATION_SOURCE_ACM | WLAN_NOTIFICATION_SOURCE_MSM, FALSE,
            reach_system_controls_wlan_notification, adapter, nullptr, &previous_sources);
...
static void reach_system_controls_stop_watching(void *userdata)
{
    reach_system_controls_adapter *adapter = static_cast<reach_system_controls_adapter *>(userdata);
    if (adapter == nullptr)
    {
...
static void reach_system_controls_destroy(void *userdata)
{
    reach_system_controls_adapter *adapter = static_cast<reach_system_controls_adapter *>(userdata);
...
extern "C" reach_result reach_windows_create_system_controls(reach_system_controls_port *out_port)
{
    if (out_port == nullptr)
    {
        return REACH_INVALID_ARGUMENT;
    ...
    *out_port = {};
    ...
    reach_system_controls_adapter *adapter = new (std::nothrow) reach_system_controls_adapter();
    if (adapter == nullptr)
    {
        return REACH_ERROR;
    }
