...
static int32_t reach_everything_load_api(reach_everything_api *api)
{
    ...
    api->set_search_w = reinterpret_cast<reach_everything_set_search_w_fn>(reach_everything_proc(api->dll, "Everything_SetSearchW"));
    api->set_regex = reinterpret_cast<reach_everything_set_regex_fn>(reach_everything_proc(api->dll, "Everything_SetRegex"));
    api->set_max = reinterpret_cast<reach_everything_set_max_fn>(reach_everything_proc(api->dll, "Everything_SetMax"));
    api->set_request_flags = reinterpret_cast<reach_everything_set_request_flags_fn>(reach_everything_proc(api->dll, "Everything_SetRequestFlags"));
    api->query_w = reinterpret_cast<reach_everything_query_w_fn>(reach_everything_proc(api->dll, "Everything_QueryW"));
    api->get_num_results = reinterpret_cast<reach_everything_get_num_results_fn>(reach_everything_proc(api->dll, "Everything_GetNumResults"));
    api->get_result_full_path_name_w = reinterpret_cast<reach_everything_get_result_full_path_name_w_fn>(reach_everything_proc(api->dll, "Everything_GetResultFullPathNameW"));
    api->get_result_attributes = reinterpret_cast<reach_everything_get_result_attributes_fn>(reach_everything_proc(api->dll, "Everything_GetResultAttributes"));
    api->get_last_error = reinterpret_cast<reach_everything_get_last_error_fn>(reach_everything_proc(api->dll, "Everything_GetLastError"));
    api->reset = reinterpret_cast<reach_everything_reset_fn>(reach_everything_proc(api->dll, "Everything_Reset"));

    if (api->set_search_w == nullptr ||
        api->set_regex == nullptr ||
        api->set_max == nullptr ||
        api->set_request_flags == nullptr ||
        api->query_w == nullptr ||
        api->get_num_results == nullptr ||
        api->get_result_full_path_name_w == nullptr ||
        api->get_result_attributes == nullptr ||
...
static void reach_search_add_candidate(std::vector<reach_search_candidate> *candidates, const uint16_t *path, int32_t is_directory)
{
    ...
    reach_search_copy_utf16(candidate.name, REACH_SEARCH_RESULT_NAME_CAPACITY, reach_search_file_name(path));
...
static reach_result reach_everything_collect(reach_search_provider *provider, const uint16_t *query, int32_t regex, std::vector<reach_search_candidate> *candidates)
{
    ...
    for (DWORD index = 0; index < count; ++index) {
        ...
        DWORD copied = api->get_result_full_path_name_w(
            index,
...
static reach_result reach_search_everything_query(reach_search_provider *provider, const uint16_t *query)
{
    ...
    size_t count = reach_search_rank_candidates(query, candidates.data(), candidates.size(), REACH_SEARCH_MAX_RESULTS);
...
static reach_result reach_search_everything_result_at(const reach_search_provider *provider, size_t index, reach_search_result *out_result)
{
    ...
    *out_result = {};
    ...
    reach_search_copy_utf16(out_result->subtitle, REACH_SEARCH_RESULT_PATH_CAPACITY, candidate->path);
