...
static const wchar_t *REACH_SERVICE_TASK_NAME = L"reachService";
...
reach_result reach_service_task_register(const wchar_t *helper_path, const wchar_t *user_id)
{
    ...
    if ((user_id == nullptr || user_id[0] == 0) &&
        reach_service_current_user_id(current_user_id, 192) != REACH_OK)
    {
        task->Release();
        reach_task_scheduler_close(&session);
        return REACH_ERROR;
    ...
    if (SUCCEEDED(hr))
    {
        BSTR task_name = SysAllocString(REACH_SERVICE_TASK_NAME);
