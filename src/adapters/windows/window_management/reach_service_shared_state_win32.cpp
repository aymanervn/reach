...
{
    reach_service_shared_reader_callback callback;
    void *user;
};
...
struct reach_shared_reader
{
    ...
    const reach_service_shared_state *view;
...
struct reach_shared_writer
{
    HANDLE mapping;
    HANDLE update_event;
    reach_service_shared_state *view;
    uint32_t generation;
...
static void reach_shared_reader_dispatch(reach_service_shared_reader_event event)
{
    reach_shared_reader_subscriber subscribers[REACH_SHARED_MAX_SUBSCRIBERS] = {};
...
static int32_t reach_shared_reader_open_mapping(void)
{
    ...
    g_reader.mapping = OpenFileMappingW(FILE_MAP_READ, FALSE, REACH_SERVICE_STATE_NAME);
    if (g_reader.mapping == nullptr)
    {
        return 0;
    }
...
static int32_t reach_shared_reader_copy_from_view(reach_service_shared_state *out_state)
{
    ...
    for (int attempt = 0; attempt < 6; ++attempt)
    {
        ...
        reach_service_shared_state snapshot = *g_reader.view;
        uint64_t after = g_reader.view->publish_sequence;
        if (before == after && (after & 1u) == 0 &&
            snapshot.version == reach_service_protocol_version() &&
            snapshot.layout_size == sizeof(reach_service_shared_state))
        {
            *out_state = snapshot;
            return 1;
        }
...
static void reach_shared_reader_set_disconnected(void)
{
    ...
    if (was_connected)
    {
        reach_shared_reader_dispatch(REACH_SERVICE_SHARED_EVENT_DISCONNECTED);
    }
}

static void reach_shared_reader_accept_state(const reach_service_shared_state *state)
{
    if (state == nullptr)
    {
        return;
    ...
    if (became_connected)
    {
        reach_shared_reader_dispatch(REACH_SERVICE_SHARED_EVENT_CONNECTED);
    }
    if (windows_changed)
    {
        reach_shared_reader_dispatch(REACH_SERVICE_SHARED_EVENT_WINDOWS_CHANGED);
    }
    if (hotkeys_changed)
    {
        reach_shared_reader_dispatch(REACH_SERVICE_SHARED_EVENT_HOTKEYS_CHANGED);
    }
...
static DWORD WINAPI reach_shared_reader_thread(void *param)
{
    ...
    for (;;)
    {
        ...
        reach_service_shared_state state = {};
                                           ...
                                           uint32_t max_windows, uint32_t *out_window_count)
{
    ...
    if (count > REACH_SERVICE_MAX_WINDOWS)
    {
        count = REACH_SERVICE_MAX_WINDOWS;
    }
    ...
    uint64_t *out_first_available, uint64_t *out_last_available)
{
    ...
    if (queue_count > REACH_SERVICE_HOTKEY_QUEUE_CAPACITY)
    {
        queue_count = REACH_SERVICE_HOTKEY_QUEUE_CAPACITY;
    }
...
reach_result reach_service_shared_writer_start(void)
{
    ...
    g_writer.update_event =
        CreateEventW(security.lpSecurityDescriptor != nullptr ? &security : nullptr, FALSE, FALSE,
                     REACH_SERVICE_UPDATE_EVENT_NAME);
    if (descriptor != nullptr)
    {
        LocalFree(descriptor);
    }
...
static int32_t reach_shared_window_snapshot_equal(const reach_service_window_snapshot *a,
                                                  const reach_service_window_snapshot *b)
{
...
static int32_t reach_shared_windows_equal(const reach_service_window_snapshot *windows,
                                                         ...
                                                         uint32_t modifiers)
{
    ...
    {
        count = REACH_SERVICE_HOTKEY_QUEUE_CAPACITY;
    }
    uint32_t index = 0;
    if (count < REACH_SERVICE_HOTKEY_QUEUE_CAPACITY)
    {
    ...
    else
    {
        ...
        g_writer.view->hotkey_queue_start =
            (g_writer.view->hotkey_queue_start + 1) % REACH_SERVICE_HOTKEY_QUEUE_CAPACITY;
    }

    reach_service_hotkey_record record = {};
