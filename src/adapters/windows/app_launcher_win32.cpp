...
static int32_t reach_window_restore_show_command(HWND hwnd)
{
    WINDOWPLACEMENT placement = {};
    placement.length = sizeof(placement);
...
static int32_t reach_is_usable_launched_window(HWND hwnd, const reach_launch_window_search *search)
{
    ...
    if (lstrcmpiW(class_name, L"ApplicationFrameWindow") == 0 ||
        ...
        lstrcmpiW(class_name, L"MozillaWindowClass") == 0 || lstrcmpiW(class_name, L"Notepad") == 0)
    {
        return 1;
    }
...
{
    reach_launch_window_search *search = reinterpret_cast<reach_launch_window_search *>(param);
    if (search == nullptr)
    {
        return FALSE;
    }
...
static HWND reach_find_launched_window(DWORD process_id, const uint16_t *path)
{
    ...
    search.path = path;
    EnumWindows(reach_find_launched_window_proc, reinterpret_cast<LPARAM>(&search));
...
static void reach_activate_launched_process_window(reach_launch_activation_context *context)
{
    ...
    {
        ShowWindow(hwnd, reach_window_restore_show_command(hwnd));
    }
...
static void reach_activate_launched_process_window_async(HANDLE process, const uint16_t *path)
{
    ...
    HANDLE thread = CreateThread(nullptr, 0, reach_activate_launched_process_window_thread, context,
                                 0, nullptr);
    if (thread != nullptr)
    {
        CloseHandle(thread);
...
{
    HWND window = nullptr;
    EnumWindows(reach_find_usable_explorer_folder_window_proc, reinterpret_cast<LPARAM>(&window));
    return window;
}
...
static int32_t reach_is_default_explorer_launch_request(const reach_app_launch_request *request)
{
    if (request == nullptr || request->path[0] == 0 || request->arguments[0] != 0 ||
        request->force_new_instance || request->run_as_admin)
    {
        return 0;
    }
...
static int32_t reach_parent_directory(const uint16_t *path, uint16_t *out_directory,
                                      DWORD out_count)
{
                                              ...
                                              const reach_app_launch_request *request)
{
    ...
    int32_t has_working_directory = reach_parent_directory(request->path, working_directory, 260);
    ...
    execute.lpParameters =
        request->arguments[0] != 0 ? reinterpret_cast<LPCWSTR>(request->arguments) : nullptr;
    execute.lpDirectory =
        has_working_directory ? reinterpret_cast<LPCWSTR>(working_directory) : nullptr;
    execute.nShow = SW_SHOW;
    if (!ShellExecuteExW(&execute))
    {
        return REACH_ERROR;
