...
static void reach_d2d_replace_all(std::string *text, const char *needle, const std::string &replacement)
{
...
static reach_result reach_d2d_load_svg_resource_text(int resource_id, reach_color color, std::string *out_svg)
{
    ...
    const char *data = loaded != nullptr ? static_cast<const char *>(LockResource(loaded)) : nullptr;
    ...
    while (cursor != nullptr && *cursor != 0) {
        ...
        if (command == 'A' || command == 'a') {
            ...
            if (figure_open) {
                ...
                segment.size = D2D1::SizeF(radius_x < 0.0f ? -radius_x : radius_x, radius_y < 0.0f ? -radius_y : radius_y);
...
static int reach_svg_view_box(const std::string &svg, float *out_x, float *out_y, float *out_width, float *out_height)
{
...
static int reach_svg_path_is_stroked(const std::string &svg, const std::string &tag)
{
...
static int reach_svg_path_in_ignored_section(const std::string &svg, size_t tag_start)
{
    size_t defs_open = svg.rfind("<defs", tag_start);
    size_t defs_close = svg.rfind("</defs>", tag_start);
    if (defs_open != std::string::npos && (defs_close == std::string::npos || defs_close < defs_open)) {
        return 1;
    }

    size_t clip_open = svg.rfind("<clipPath", tag_start);
    size_t clip_close = svg.rfind("</clipPath>", tag_start);
    return clip_open != std::string::npos && (clip_close == std::string::npos || clip_close < clip_open);
}

static D2D1_MATRIX_3X2_F reach_svg_root_transform(const std::string &svg, float view_x, float view_y, float view_width, float view_height)
{
    size_t svg_start = svg.find("<svg");
    size_t svg_end = svg_start != std::string::npos ? svg.find('>', svg_start) : std::string::npos;
...
static float reach_svg_inherited_stroke_width(const std::string &svg, size_t tag_start, float fallback)
{
    size_t group_start = svg.rfind("<g", tag_start);
    size_t group_close = svg.rfind("</g>", tag_start);
    if (group_start == std::string::npos || (group_close != std::string::npos && group_close > group_start)) {
        return fallback;
    }
...
{
    ID2D1RenderTarget *target = reach_d2d_target(backend);
    if (backend == nullptr || backend->factory == nullptr || target == nullptr || command == nullptr) {
        return REACH_INVALID_ARGUMENT;
    }
    ...
    D2D1_MATRIX_3X2_F transform =
        ...
        D2D1::Matrix3x2F::Scale(scale_x, scale_y) *
        D2D1::Matrix3x2F::Translation(command->rect.x - view_x * scale_x, command->rect.y - view_y * scale_y);
    ...
    while (result == REACH_OK) {
        ...
        if (reach_svg_path_is_stroked(svg, tag)) {
            float stroke_width = reach_svg_attribute_float(tag, "stroke-width", reach_svg_inherited_stroke_width(svg, tag_start, 1.0f));
...
static reach_result reach_d2d_draw_vector_icon(reach_render_backend *backend, const reach_render_command *command)
{
    ...
    std::string svg_text;
    reach_result load_result = reach_d2d_load_svg_resource_text(resource_id, command->color, &svg_text);
