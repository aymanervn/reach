                                            ...
                                            float stroke_width, float end_alpha = 0.0f)
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = target->CreateLinearGradientBrush(D2D1::LinearGradientBrushProperties(start, end),
                                               stop_collection, &brush);
    }
...
static HRESULT reach_d2d_draw_gradient_arc(ID2D1RenderTarget *target, D2D1_POINT_2F start,
                                           D2D1_POINT_2F end, D2D1_SIZE_F radius,
                                           D2D1_SWEEP_DIRECTION sweep, reach_color color,
                                           float stroke_width, float end_alpha = 0.85f)
{
                                                 ...
                                                 const reach_render_command *command)
{
    ...
    D2D1_ROUNDED_RECT clip_rect =
        D2D1::RoundedRect(D2D1::RectF(command->clip_rect.x, command->clip_rect.y,
                                      command->clip_rect.x + command->clip_rect.width,
                                      command->clip_rect.y + command->clip_rect.height),
                          command->clip_radius, command->clip_radius);
    ...
    if (SUCCEEDED(hr))
    {
        D2D1_LAYER_PARAMETERS parameters =
            D2D1::LayerParameters(D2D1::RectF(command->clip_rect.x, command->clip_rect.y,
                                              command->clip_rect.x + command->clip_rect.width,
                                              command->clip_rect.y + command->clip_rect.height),
                                  clip_geometry);
