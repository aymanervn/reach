                                       ...
                                       const reach_render_command *command)
{
    ...
    if (SUCCEEDED(hr) && text_length > 0)
    {
        ...
        hr = backend->text_factory->CreateTextLayout(text, text_length, format, command->rect.width,
                                                     command->rect.height, &layout);
    ...
    caret.rect = D2D1::RectF(caret_x, command->rect.y, caret_x + 2.0f,
                             command->rect.y + command->rect.height);
