...
static reach_result reach_ui_state_delete_launcher_range(reach_ui_state *state, size_t start, size_t end)
{
...
reach_result reach_ui_state_delete_launcher_previous_character(reach_ui_state *state)
{
    ...
    return reach_ui_state_delete_launcher_range(state, state->launcher.caret_index - 1, state->launcher.caret_index);
}
...
reach_result reach_ui_state_delete_launcher_next_character(reach_ui_state *state)
{
    ...
    return reach_ui_state_delete_launcher_range(state, state->launcher.caret_index, state->launcher.caret_index + 1);
}
...
reach_result reach_ui_state_delete_launcher_previous_word(reach_ui_state *state)
{
    ...
    while (start > 1 &&
           reach_core_is_word_space(state->launcher.query[start - 1]) &&
...
reach_result reach_ui_state_move_launcher_caret_left(reach_ui_state *state, int32_t by_word)
{
    ...
    while (state->launcher.caret_index > 0 && reach_core_is_word_space(state->launcher.query[state->launcher.caret_index - 1])) {
        state->launcher.caret_index -= 1;
    }
    while (state->launcher.caret_index > 0 && !reach_core_is_word_space(state->launcher.query[state->launcher.caret_index - 1])) {
        state->launcher.caret_index -= 1;
