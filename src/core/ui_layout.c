                                     ...
                                     reach_ui_layout *out_layout)
{
    ...
    out_layout->dock.power_button.x = dock_width - icon_size - gap;
    ...
    out_layout->dock.system_separator.y = (dock_height - separator_height) * 0.5f;
    ...
    out_layout->dock.tray_button.x = out_layout->dock.quick_settings_button.x - icon_size;
    ...
    out_layout->launcher.search_box.x =
        input->monitor_bounds.x +
        (input->monitor_bounds.width - out_layout->launcher.search_box.width) * 0.5f;
    ...
    float total_width =
        (launcher_icon * (float)state->pinned_app_count) +
        (launcher_gap * (float)(state->pinned_app_count > 0 ? state->pinned_app_count - 1 : 0));
    float apps_x = input->monitor_bounds.x + (input->monitor_bounds.width - total_width) * 0.5f;
    float apps_y = out_layout->launcher.search_box.y + out_layout->launcher.search_box.height +
                   reach_scale(24.0f, scale);

    for (size_t index = 0; index < state->pinned_app_count; ++index)
    {
        out_layout->launcher.pinned_app_slots[index].x =
    ...
    if (state->launcher.result_count > 0)
    {
        ...
        out_layout->launcher.bounds.height = out_layout->launcher.search_results.y +
                                             out_layout->launcher.search_results.height -
                                             out_layout->launcher.bounds.y;
    }
    else if (out_layout->launcher.pinned_app_slot_count > 0)
    {
        float launcher_left =
            apps_x < out_layout->launcher.search_box.x ? apps_x : out_layout->launcher.search_box.x;
        float launcher_right = apps_x + total_width;
        float search_right =
            out_layout->launcher.search_box.x + out_layout->launcher.search_box.width;
