...
reach_result reach_app_controller_tick(reach_app_controller *controller, const reach_app_controller_tick_desc *tick)
{
    ...
    reach_result result = reach_ui_layout_compute(&controller->ui, &tick->layout_input, &controller->layout);
    return result == REACH_OK ? reach_ui_build_render_commands(&controller->ui, &controller->layout, &controller->render_commands) : result;
}
