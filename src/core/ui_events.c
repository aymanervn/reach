...
static void reach_ui_copy_query(uint16_t *dst, const reach_ui_state *state)
{
    ...
    for (size_t index = 0; index <= state->launcher.query_length && index <= REACH_MAX_SEARCH_CHARS; ++index) {
...
reach_result reach_ui_handle_event(reach_ui_state *state, const reach_ui_event *event, reach_ui_intent *out_intent)
{
    ...
    case REACH_UI_EVENT_ARROW_UP:
        return state->launcher.open ? reach_ui_state_select_previous_launcher_result(state) : REACH_OK;
    ...
    case REACH_UI_EVENT_ARROW_LEFT:
        return state->launcher.open ? reach_ui_state_move_launcher_caret_left(state, (event->modifiers & REACH_UI_EVENT_MODIFIER_CTRL) != 0) : REACH_OK;
    case REACH_UI_EVENT_ARROW_RIGHT:
        return state->launcher.open ? reach_ui_state_move_launcher_caret_right(state, (event->modifiers & REACH_UI_EVENT_MODIFIER_CTRL) != 0) : REACH_OK;
