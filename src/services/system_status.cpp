...
/*
 * System-status service. The two worker bodies are verbatim relocations of
 * composition's quick-settings audio/system refresh threads; the generation
 * bookkeeping ("drop a snapshot a newer refresh superseded") moved inside so
 * consumers only ever see current snapshots.
 */
