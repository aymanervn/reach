...
/*
 * Window-tracking service. The snapshot diff, focus history, and the async
 * window-control worker are verbatim relocations from composition's
 * host_window_tracking.cpp (they were feature-shared state living on
 * reach_host); the dock/render-icon effects they used to apply inline are
 * reported back through reach_window_tracking_refresh_report instead.
 */
