...
static void reach_pin_ensure_ids(reach_config_snapshot *snapshot)
{
    if (snapshot == nullptr)
    {
        return;
...
static reach_result reach_pin_add_default_explorer(reach_config_snapshot *snapshot)
{
    ...
    *app = {};
    ...
    const uint16_t explorer_title[] = {'e', 'x', 'p', 'l', 'o', 'r', 'e', 'r', 0};
...
{
    if (path == nullptr || path[0] == 0 || app_user_model_id == nullptr ||
