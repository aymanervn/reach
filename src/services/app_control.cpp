...
/*
 * App-control service. Launch lane: independent fire-and-forget tasks — a
 * FIFO queue feeds a bounded pool of detached workers sharing a refcounted
 * state block, so a hung launch neither delays another nor blocks shutdown
 * (abandoned workers finish against the port copy in the block; the
 * app-launcher port contract requires launch() to be instance-state-free).
 * Window lane: one joined worker running activate/minimize/close batches
 * through the window-manager port, starting the privileged-control path on
 * demand; joined because that port is stateful and must outlive every call.
 */
...
/* Idle launch workers exit after this long without work. */
...
struct reach_app_control_launch_state
{
    ...
    int32_t stop = 0;
    int32_t refs = 1; /* the service handle; each worker holds one more */
};
...
/* Runs on the window worker: elevate through the privileged-control path when
   it is not yet available, then dispatch. */
                                               ...
                                               const reach_app_launch_request *request)
{
    ...
    if (spawn)
    {
        ...
        catch (...)
        {
            std::lock_guard<std::mutex> lock(state->mutex);
            --state->total_workers;
            --state->refs; /* the service ref still holds the block alive */
            if (state->queue_count > 0)
            {
                --state->queue_count; /* nobody may come for it */
            }
            return REACH_ERROR;
        }
    }
