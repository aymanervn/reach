...
/*
 * Config service. The worker body is a verbatim relocation of composition's
 * config-reload thread: one pending job slot (latest-wins), pin mutations
 * reload the snapshot afterwards so consumers always apply current state.
 */
