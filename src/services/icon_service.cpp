...
/*
 * Icon service — pull-model keyed cache (see the header for the contract).
 * One mutex guards the cache + the load queue + the evicted list; loads run
 * on the worker OUTSIDE the lock (the per-entry `loading` flag dedupes).
 * Icon counts are small (bounded by pinned apps, open windows, and visible
 * search results), so a linear-scanned vector keeps the cache simple.
 */
...
typedef struct reach_icon_cache_entry
{
    std::wstring path;
    int32_t size_px = 0;
    uint64_t id = 0;               /* render icon id; 0 while loading / failed */
    reach_icon_handle handle = {}; /* provider handle (for release) */
    int32_t loading = 0;
...
struct reach_icon_service
{
    ...
    int32_t stop = 0;
    int32_t in_flight = 0;
    /* A load landed since the last take (successful or negative-cached);
       the consumer redraws its icon surfaces into cache hits. */
...
static void reach_icon_service_thread_main(reach_icon_service *service)
{
    for (;;)
    {
        ...
        void *notify_user = nullptr;
        {
            ...
            {
                /* The entry vanished (clear raced the load): drop the icon. */
            ...
            {
                /* Negative-cache: per-frame gets must not retry-storm. */
                                ...
                                int32_t size_px)
{
    ...
    int32_t queued = 0;
    {
        ...
        if (entry != nullptr)
        {
            ...
            /* Failure cooldown expired: retry. */
            entry->loading = 1;
...
void reach_icon_service_clear(reach_icon_service *service)
{
    ...
    service->jobs.clear();
    /* Shutdown path: renderer teardown frees the images; drop the queue. */
