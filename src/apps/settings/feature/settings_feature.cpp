...
#define REACH_SETTINGS_SELECTOR_WIDTH 318.0f
#define REACH_SETTINGS_SELECTOR_WIDE_WIDTH 520.0f
                                                ...
                                                float height, float selector_width_dp, float scale)
{
    *card = reach_settings_rect(x, y, width, height);
    ...
    if (width >= REACH_SETTINGS_SELECTOR_WIDE_WIDTH * scale)
    {
        ...
        *subtitle = reach_settings_rect(text_x, y + 37.0f * scale,
                                        selector_x - text_x - 16.0f * scale, 16.0f * scale);
        *selector = reach_settings_rect(selector_x, y + (height - selector_height) * 0.5f,
    ...
    *selector = reach_settings_rect(x + 16.0f * scale, y + height - selector_height - 10.0f * scale,
                                                       ...
                                                       reach_settings_model *model)
{
    ...
    if (model != nullptr && model->selected_page == REACH_SETTINGS_PAGE_DISPLAY)
    {
        ...
        float theme_card_height =
            area_width >= REACH_SETTINGS_SELECTOR_WIDE_WIDTH * scale ? card_height : 104.0f * scale;
        reach_settings_layout_selector_card(
            &layout.display_theme_card, &layout.display_theme_title, &layout.display_theme_subtitle,
            &layout.display_theme_selector, &layout.display_theme_icon, area_x, theme_card_y,
            area_width, theme_card_height, 176.0f, scale);
        ...
        float appearance_card_height = area_width >= REACH_SETTINGS_SELECTOR_WIDE_WIDTH * scale
                                           ? 76.0f * scale
                                           : 104.0f * scale;
        reach_settings_layout_selector_card(
        ...
        reach_settings_layout_selector_card(
            &layout.top_bar_style_card, &layout.top_bar_style_title, &layout.top_bar_style_subtitle,
            &layout.top_bar_style_selector, nullptr, area_x, desktop_y, area_width,
            appearance_card_height, REACH_SETTINGS_SELECTOR_WIDTH, scale);
                                                  ...
                                                  float y)
{
    ...
    {
        result.type = REACH_SETTINGS_HIT_TOP_BAR_AUTO_HIDE_TOGGLE;
        return result;
    }
