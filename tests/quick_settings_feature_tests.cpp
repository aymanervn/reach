...
static const size_t project_tile_command_offset = 3;
...
static reach_quick_settings_model test_model_with_output_devices(size_t count)
{
    ...
    devices.count = count;
    for (size_t index = 0; index < count && index < REACH_AUDIO_VOLUME_MAX_OUTPUT_DEVICES; ++index)
    {
        devices.devices[index].icon_id = (uint64_t)(7000 + index);
        devices.devices[index].is_default = index == 1 ? 1 : 0;
        copy_ascii(devices.devices[index].device_id, REACH_AUDIO_VOLUME_DEVICE_ID_CAPACITY,
                   index == 0 ? "device-a" : "device-b");
        copy_ascii(devices.devices[index].label, REACH_AUDIO_VOLUME_DEVICE_LABEL_CAPACITY,
...
static void set_system_states(reach_quick_settings_model *model, reach_network_kind network_kind,
                              int32_t network_connected, const char *network_label,
                              int32_t signal_strength, int32_t bluetooth_enabled,
                              int32_t has_battery, int32_t battery_saver_on,
                              int32_t brightness_available)
{
    ...
    reach_quick_settings_model_set_system_states(model, &network, &bluetooth, &power, &brightness);
}

static reach_quick_settings_layout test_layout_for_model(const reach_quick_settings_model *model)
{
...
static void test_layout_places_volume_pill_above_output_device_and_expand(void)
{
    ...
    expect_true(layout.expand_button.x >= bounds.x &&
    ...
    expect_near(layout.main_volume_pill.bounds.x - bounds.x, expected_padding, 0.001f,
                "left content padding is compact");
    expect_near(bounds.x + bounds.width -
                    (layout.main_volume_pill.bounds.x + layout.main_volume_pill.bounds.width),
                expected_padding, 0.001f, "right content padding is compact");
    expect_near(bounds.y + bounds.height - (layout.expand_button.y + layout.expand_button.height),
                expected_padding, 0.001f, "bottom content padding is compact");

    expect_near(layout.main_volume_pill.bounds.width, layout.expand_button.width, 0.001f,
                "volume pill and expand button have same width");
    expect_near(layout.output_device_button.width, layout.main_volume_pill.bounds.width, 0.001f,
                "output device button and master pill have same width");
    expect_near(layout.main_slider_track.height, layout.main_volume_pill.bounds.height, 0.001f,
                "slider track fills volume pill height");

    expect_near(layout.main_volume_pill.header_icon.width, 22.0f, 0.001f,
                "master volume icon uses header icon size");
}

static void test_desktop_system_grid_shows_network_bluetooth_and_project(void)
{
    reach_quick_settings_model model = {};
    reach_quick_settings_model_init(&model);
    ...
    expect_true(layout.system_tile_count == 3, "desktop grid exposes three system tiles");
    ...
    expect_true(layout.project_tile.bounds.width > 0.0f, "desktop grid shows project tile");
    expect_near(layout.battery_saver_tile.bounds.width, 0.0f, 0.001f,
                "battery saver hidden without battery");
    expect_true(layout.project_tile.bounds.y > layout.network_tile.bounds.y,
                "project is in second grid row without battery");
    expect_near(layout.project_tile.bounds.x, layout.network_tile.bounds.x, 0.001f,
                "project aligns to first column without battery");
    expect_near(layout.brightness_pill.bounds.height, 0.0f, 0.001f,
                "brightness hidden when unavailable");
}
...
static void test_battery_system_grid_is_two_by_two(void)
{
    reach_quick_settings_model model = {};
    reach_quick_settings_model_init(&model);
    set_system_states(&model, REACH_NETWORK_KIND_WIFI, 1, "Reach WiFi", 84, 1, 1, 1, 0);

    reach_quick_settings_layout layout = test_layout_for_model(&model);

    expect_true(layout.system_tile_count == 4, "battery grid exposes four system tiles");
    expect_near(layout.network_tile.bounds.width, layout.bluetooth_tile.bounds.width, 0.001f,
                "grid columns have matching widths");
    expect_true(layout.battery_saver_tile.bounds.y > layout.network_tile.bounds.y,
                "battery saver is in second grid row");
    expect_true(layout.project_tile.bounds.y > layout.bluetooth_tile.bounds.y,
                "project is in second grid row");
    expect_near(layout.battery_saver_tile.bounds.x, layout.network_tile.bounds.x, 0.001f,
                "battery saver aligns to first column");
    expect_near(layout.project_tile.bounds.x, layout.bluetooth_tile.bounds.x, 0.001f,
...
static void test_brightness_available_shows_slider_and_hit_action(void)
{
    reach_quick_settings_model model = {};
    reach_quick_settings_model_init(&model);
    set_system_states(&model, REACH_NETWORK_KIND_NONE, 0, nullptr, 0, 0, 0, 0, 1);

    reach_quick_settings_layout layout = test_layout_for_model(&model);

    expect_true(layout.brightness_pill.bounds.height > 0.0f, "brightness available shows slider");
    expect_true(layout.brightness_pill.bounds.y >
                    layout.system_grid_bounds.y + layout.system_grid_bounds.height,
                "brightness slider appears below system grid");
    expect_true(layout.main_volume_pill.bounds.y >
                    layout.brightness_pill.bounds.y + layout.brightness_pill.bounds.height,
                "master volume remains below brightness");

    float x = layout.brightness_slider_track.x + layout.brightness_slider_track.width * 0.25f;
    float y = layout.brightness_slider_track.y + layout.brightness_slider_track.height * 0.5f;
    reach_quick_settings_hit_result hit = reach_quick_settings_hit_test(&layout, &model, x, y);
    reach_quick_settings_action action = reach_quick_settings_action_for_hit(hit);

    expect_true(hit.type == REACH_QUICK_SETTINGS_HIT_BRIGHTNESS_SLIDER, "brightness slider hit");
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_SET_BRIGHTNESS,
...
static void test_slider_hit_maps_to_volume(void)
{
    ...
    reach_quick_settings_hit_result mid = reach_quick_settings_hit_test(&layout, nullptr, mid_x, y);
...
static void test_expand_button_hit_maps_to_expand_action(void)
{
    ...
    reach_quick_settings_hit_result hit = reach_quick_settings_hit_test(&layout, nullptr, x, y);

    reach_quick_settings_action action = reach_quick_settings_action_for_hit(hit);

    expect_true(hit.type == REACH_QUICK_SETTINGS_HIT_EXPAND_BUTTON, "expand button hit");
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_EXPAND,
                "expand hit maps to expand action");
}
...
static void test_output_device_button_hit_maps_to_toggle_action(void)
{
    ...
    expect_true(hit.type == REACH_QUICK_SETTINGS_HIT_OUTPUT_DEVICE_BUTTON,
                "output device button hit");
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_TOGGLE_OUTPUT_DEVICES,
...
static void test_output_device_title_chevron_hit_maps_to_toggle_action(void)
{
    ...
    reach_quick_settings_layout layout = test_layout_for_model(&model);
    float x =
        layout.output_devices_title_chevron.x + layout.output_devices_title_chevron.width * 0.5f;
    float y =
        layout.output_devices_title_chevron.y + layout.output_devices_title_chevron.height * 0.5f;
...
static void test_slider_hit_maps_to_set_volume_action(void)
{
    ...
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_SET_MAIN_VOLUME,
                "slider hit maps to set volume action");
...
static void test_system_tile_hits_map_to_actions(void)
{
    reach_quick_settings_model model = {};
    reach_quick_settings_model_init(&model);
    set_system_states(&model, REACH_NETWORK_KIND_ETHERNET, 1, "Ethernet", 0, 0, 1, 0, 0);
    reach_quick_settings_layout layout = test_layout_for_model(&model);

    reach_quick_settings_hit_result hit = reach_quick_settings_hit_test(
        &layout, &model, layout.network_tile.bounds.x + layout.network_tile.bounds.width * 0.5f,
    ...
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_NETWORK_TILE,
                "network tile maps to network action intent");

    hit = reach_quick_settings_hit_test(
        &layout, &model, layout.bluetooth_tile.bounds.x + layout.bluetooth_tile.bounds.width * 0.5f,
    ...
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_TOGGLE_BLUETOOTH,
                "bluetooth tile maps to toggle action");
    ...
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_TOGGLE_BATTERY_SAVER,
                "battery saver tile maps to toggle action");

    hit = reach_quick_settings_hit_test(
        &layout, &model, layout.project_tile.bounds.x + layout.project_tile.bounds.width * 0.5f,
    ...
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_OPEN_PROJECT,
                "project tile maps to open project action");
}
...
static void test_expanded_layout_shows_app_volume_panel(void)
{
    ...
    expect_true(layout.app_volumes_title.y >
                    layout.output_device_button.y + layout.output_device_button.height,
                "app volumes title is below output device button");
    expect_true(layout.app_volumes_panel.y >
                    layout.app_volumes_title.y + layout.app_volumes_title.height,
                "app volumes panel is below title");
    expect_near(layout.app_volumes_panel.width, layout.main_volume_pill.bounds.width, 0.001f,
                "app volumes panel matches master pill width");
    expect_near(layout.app_volumes_panel.width, layout.expand_button.width, 0.001f,
                "app volumes panel matches expand width");
    expect_near(layout.app_volumes_panel.height, 120.0f, 0.001f,
                "app volumes panel height includes visible rows and hide row");
    expect_true(layout.expand_button.y >= layout.app_volumes_panel.y &&
    ...
    expect_near(layout.expand_button.y,
                layout.app_volume_rows[1].bounds.y + layout.app_volume_rows[1].bounds.height,
                0.001f, "hide button follows app volume rows");
}
...
static void test_expanded_output_device_layout_shows_panel(void)
{
    ...
    expect_true(layout.output_device_button.height == 0.0f,
                "expanded output layout hides collapsed output button");
    expect_true(layout.output_devices_title.y >
                    layout.main_volume_pill.bounds.y + layout.main_volume_pill.bounds.height,
                "output devices title is below master volume");
    expect_true(layout.output_devices_panel.y >
                    layout.output_devices_title.y + layout.output_devices_title.height,
                "output devices panel is below title");
    expect_near(layout.output_devices_panel.width, layout.main_volume_pill.bounds.width, 0.001f,
                "output devices panel matches master pill width");
    expect_near(layout.output_devices_panel.height, 88.0f, 0.001f,
                "output devices panel height follows device rows");
    expect_true(layout.output_device_rows[0].icon.x < layout.output_device_rows[0].label.x,
                "output device icon is before label");
    expect_true(layout.output_device_rows[0].label.x + layout.output_device_rows[0].label.width <
...
static void test_output_device_row_hit_maps_to_set_device_action(void)
{
    ...
    float x =
        layout.output_device_rows[1].bounds.x + layout.output_device_rows[1].bounds.width * 0.5f;
    float y =
        layout.output_device_rows[1].bounds.y + layout.output_device_rows[1].bounds.height * 0.5f;

    reach_quick_settings_hit_result hit = reach_quick_settings_hit_test(&layout, &model, x, y);
    reach_quick_settings_action action = reach_quick_settings_action_for_hit(hit);

    expect_true(hit.type == REACH_QUICK_SETTINGS_HIT_OUTPUT_DEVICE_ROW, "output device row hit");
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_SET_OUTPUT_DEVICE,
                "output device row maps to set output action");
    expect_true(action.output_device_index == 1, "output action carries device index");
    expect_true(text_equals_ascii(action.output_device_id, "device-b"),
...
static void test_app_volume_row_layout_is_compact(void)
{
    ...
    expect_near(row->bounds.height, 40.0f, 0.001f, "app volume row uses compact fixed height");
    expect_true(row->app_icon.x < row->app_label.x, "app row icon is before label");
    expect_true(row->app_label.x + row->app_label.width < row->slider_full_range_line.x,
                "app row label is before slider");
    expect_true(row->slider_full_range_line.x + row->slider_full_range_line.width <
                    row->app_volume_percent.x,
                "app row slider is before percent");
    expect_true(row->slider_full_range_line.x < row->slider_thumb.x + row->slider_thumb.width,
                "app row slider is before thumb edge");
    expect_near(row->slider_level_line.width,
                row->slider_full_range_line.width * model.sessions.sessions[1].level, 0.001f,
...
static void test_session_slider_hit_maps_to_session_action(void)
{
    ...
    float y = layout.app_volume_rows[1].bounds.y + layout.app_volume_rows[1].bounds.height * 0.5f;

    reach_quick_settings_hit_result hit = reach_quick_settings_hit_test(&layout, &model, x, y);
    reach_quick_settings_action action = reach_quick_settings_action_for_hit(hit);

    expect_true(hit.type == REACH_QUICK_SETTINGS_HIT_SESSION_SLIDER, "session slider hit");
    expect_true(action.type == REACH_QUICK_SETTINGS_ACTION_SET_SESSION_VOLUME,
                "session hit maps to session action");
    expect_true(action.session_index == 1, "session action carries index");
    expect_true(text_equals_ascii(action.session_instance_id, "session-b"),
                "session action carries key");
...
static void test_session_list_cap_is_respected(void)
{
    ...
    expect_true(layout.app_volume_row_count == REACH_AUDIO_VOLUME_MAX_SESSIONS,
                "layout caps app volume rows");
}
...
static void test_expanded_render_emits_app_volume_panel_rows(void)
{
    ...
    expect_true(
        commands.count >= 33,
        "expanded render emits grid, master, output button, panel rows, and collapse commands");
    const size_t app_title = 15 + project_tile_command_offset;
    const size_t app_panel = 16 + project_tile_command_offset;
    expect_true(commands.commands[app_title].type == REACH_RENDER_COMMAND_TEXT &&
                    text_equals_ascii(commands.commands[app_title].text, "App volumes"),
    ...
    expect_true(commands.commands[32 + project_tile_command_offset].type ==
                        REACH_RENDER_COMMAND_VECTOR_ICON &&
                    commands.commands[32 + project_tile_command_offset].icon_id ==
...
static void test_expanded_output_device_render_emits_checkmark(void)
{
    ...
    expect_true(commands.commands[10 + project_tile_command_offset].type ==
                        REACH_RENDER_COMMAND_TEXT &&
                    text_equals_ascii(commands.commands[10 + project_tile_command_offset].text,
                                      "Output devices"),
                "expanded output title replaces output button");
    expect_true(commands.commands[11 + project_tile_command_offset].type ==
                        REACH_RENDER_COMMAND_VECTOR_ICON &&
    ...
    expect_true(commands.commands[18 + project_tile_command_offset].type ==
                        REACH_RENDER_COMMAND_VECTOR_ICON &&
                    commands.commands[18 + project_tile_command_offset].icon_id ==
                        REACH_VECTOR_ICON_CHECK,
...
static void test_system_grid_render_emits_labels_icons_and_active_colors(void)
{
    ...
    reach_quick_settings_model_init(&model);
    reach_quick_settings_model_set_main_volume(&model, 0.5f, 0);
    set_system_states(&model, REACH_NETWORK_KIND_WIFI, 1, "Reach WiFi", 82, 1, 1, 1, 0);
    ...
    expect_true(commands.commands[1].type == REACH_RENDER_COMMAND_VECTOR_ICON &&
                    commands.commands[1].icon_id == REACH_VECTOR_ICON_WIFI_HIGH,
                "wifi network uses high strength icon");
    expect_true(commands.commands[2].type == REACH_RENDER_COMMAND_TEXT &&
    ...
    expect_true(commands.commands[7].type == REACH_RENDER_COMMAND_VECTOR_ICON &&
    ...
    expect_true(commands.commands[11].type == REACH_RENDER_COMMAND_TEXT &&
...
static void test_render_emits_volume_pill_and_expand_commands(void)
{
    ...
    expect_true(commands.count >= 18,
                "render emits grid, volume row, slider, button, label, icon commands");

    expect_true(commands.commands[0].type == REACH_RENDER_COMMAND_RECT,
    ...
    const size_t volume_icon = 6 + project_tile_command_offset;
    const size_t volume_label = 7 + project_tile_command_offset;
    const size_t slider_track = 8 + project_tile_command_offset;
    const size_t slider_fill = 9 + project_tile_command_offset;
    const size_t output_button = 10 + project_tile_command_offset;
    const size_t output_button_arrow = 14 + project_tile_command_offset;
    const size_t expand_button = 15 + project_tile_command_offset;
    const size_t expand_button_arrow = 17 + project_tile_command_offset;

    expect_true(commands.commands[volume_icon].type == REACH_RENDER_COMMAND_VECTOR_ICON &&
                    commands.commands[volume_icon].icon_id == REACH_VECTOR_ICON_VOLUME_HIGH,
                "first command is volume icon");
    expect_color_equal(commands.commands[volume_icon].color, theme.icon_backplate_background,
                       "volume icon uses app backplate color");

    expect_true(commands.commands[volume_label].type == REACH_RENDER_COMMAND_TEXT &&
                    text_equals_ascii(commands.commands[volume_label].text, "Master volume"),
                "second command is Master volume text");

    expect_true(commands.commands[slider_track].type == REACH_RENDER_COMMAND_RECT,
    ...
    expect_near(commands.commands[output_button].radius, reach_popup_radius(), 0.001f,
                "output device button uses popup corner radius");
    expect_true(commands.commands[output_button_arrow].type == REACH_RENDER_COMMAND_VECTOR_ICON &&
                    commands.commands[output_button_arrow].icon_id == REACH_VECTOR_ICON_ARROW_DOWN,
                "output device button uses arrow down icon");

    expect_true(commands.commands[expand_button].type == REACH_RENDER_COMMAND_RECT,
                "ninth command is expand button background");
    expect_near(commands.commands[expand_button].radius, reach_popup_radius(), 0.001f,
