...
static void test_insert_and_caret()
{
    ...
    expect_true(reach_test_utf16_equals_ascii(edit.text, "hi"), "buffer is 'hi'");
    ...
    expect_true(reach_test_utf16_equals_ascii(edit.text, "hXi"), "insert in middle -> 'hXi'");
}
...
static void test_backspace_delete_home_end()
{
    ...
    reach_copy_ascii_to_utf16(buf, REACH_TEXT_EDIT_CAPACITY, "abc");
    ...
    expect_true(reach_test_utf16_equals_ascii(edit.text, "ab"), "backspace -> 'ab'");
    ...
    expect_true(reach_test_utf16_equals_ascii(edit.text, "b"), "delete -> 'b'");
...
static void test_selection_and_copy()
{
    ...
    reach_text_edit_init(&edit, 0);
    uint16_t buf[REACH_TEXT_EDIT_CAPACITY];
    reach_copy_ascii_to_utf16(buf, REACH_TEXT_EDIT_CAPACITY, "hello");
    reach_text_edit_set_text(&edit, buf);
    ...
    expect_true(copied == 5 && reach_test_utf16_equals_ascii(out, "hello"),
                "copy selection -> 'hello'");

    reach_text_edit_insert_char(&edit, 'Z');
    expect_true(reach_test_utf16_equals_ascii(edit.text, "Z"),
                "insert over selection replaces all");

    reach_copy_ascii_to_utf16(buf, REACH_TEXT_EDIT_CAPACITY, "abcd");
    reach_text_edit_set_text(&edit, buf);
    reach_text_edit_handle_key(&edit, REACH_TEXT_EDIT_KEY_HOME, none());
    ...
    expect_true(reach_text_edit_delete_selection(&edit) == 1 &&
                    reach_test_utf16_equals_ascii(edit.text, "cd"),
                "delete selection -> 'cd'");
}
...
static void test_paste_and_events()
{
    ...
    reach_text_edit_init(&edit, 0);
    uint16_t buf[REACH_TEXT_EDIT_CAPACITY];
    reach_copy_ascii_to_utf16(buf, REACH_TEXT_EDIT_CAPACITY, "a\tb\nc");
    expect_true(reach_text_edit_insert_text(&edit, buf) == REACH_TEXT_EDIT_EVENT_TEXT_CHANGED,
                "paste emits TEXT_CHANGED");
    expect_true(reach_test_utf16_equals_ascii(edit.text, "abc"),
                "paste strips tab/newline -> 'abc'");
