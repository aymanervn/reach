...
int main(void)
{
    expect_true(reach_window_identity_is_explorer_dialog(wide(L"C:\\Windows\\explorer.exe"),
                                                         wide(L"OperationStatusWindow")),
                "copy operation window is an Explorer dialog");
    expect_true(reach_window_identity_is_explorer_dialog(wide(L"c:\\windows\\EXPLORER.EXE"),
                                                         wide(L"#32770")),
                "standard Explorer dialog matching is case insensitive");
    expect_true(!reach_window_identity_is_explorer_dialog(wide(L"C:\\Windows\\explorer.exe"),
                                                          wide(L"CabinetWClass")),
                "Explorer folder windows are not dialogs");
    expect_true(!reach_window_identity_is_explorer_dialog(wide(L"C:\\Tools\\explorer.exe.backup"),
                                                          wide(L"OperationStatusWindow")),
                "similarly named executables are rejected");
    expect_true(
        !reach_window_identity_is_explorer_dialog(wide(L"C:\\Tools\\other.exe"), wide(L"#32770")),
